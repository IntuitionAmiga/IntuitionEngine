...
host_goos="${HOST_SDK_GOOS:-linux}"
package_name="${HOST_SDK_NAME:-intuition-engine-host-sdk-${host_goos}-${host_arch}}"
target_goarch="${HOST_SDK_GOARCH:-${host_arch}}"
host_cc="${HOST_SDK_CC:-cc}"
host_cc_flags="${HOST_SDK_CC_FLAGS:-}"
host_make_cc="${host_cc}"
host_objdump="${HOST_SDK_OBJDUMP:-}"
go_command="${HOST_SDK_GO:-go}"
qemu_aarch64="${HOST_SDK_QEMU_AARCH64:-}"
host_sysroot="${HOST_SDK_SYSROOT:-}"
host_cflags=''
host_ldflags=''
binary_suffix="${HOST_SDK_BINARY_SUFFIX:-}"
archive_format="${HOST_SDK_ARCHIVE_FORMAT:-tar.xz}"
...
if [[ "${host_arch}" == arm64 ]]; then
...
elif [[ "${host_goos}" == windows ]]; then
    [[ "${target_goarch}" == amd64 ]] || fail "Windows distribution requires GOARCH=amd64"
    command -v "${host_cc}" >/dev/null 2>&1 || fail "Windows compiler is unavailable: ${host_cc}"
    [[ -n "${host_objdump}" ]] || host_objdump="$(command -v x86_64-w64-mingw32-objdump || true)"
    [[ -n "${host_objdump}" ]] || fail "MinGW objdump is unavailable"
    ...
    host_gcc_libdir="$(${host_cc} -print-file-name=libgcc.a)"
    [[ "${host_gcc_libdir}" != libgcc.a ]] || fail "MinGW libgcc.a is unavailable"
    host_gcc_libdir="${host_gcc_libdir%/*}"
    binary_suffix=".exe"
    archive_format="zip"
else
    [[ "${host_goos}" == linux ]] || fail "unsupported Host SDK host OS: ${host_goos}"
fi
...
if [[ "${host_goos}" == windows ]]; then
    go_build=(env CGO_ENABLED=0 GOOS=windows GOARCH="${target_goarch}" "${go_command}" build)
else
    go_build=(env GOOS=linux GOARCH="${target_goarch}" "${go_command}" build)
fi
run_host_tool() {
    if [[ "${host_arch}" == arm64 ]]; then
...
build_external_tool() {
    ...
    if [[ "${name}" == qbe ]]; then
        ...
        if [[ "${host_goos}" == windows || "${host_arch}" == arm64 ]]; then
        ...
        if [[ "${host_goos}" == windows ]]; then
            ...
            (cd "${build_dir}" && CC="${host_make_cc}" ./configure --host=x86_64-w64-mingw32 --target=x86_64-linux-gnu --with-gcc-libdir="${host_gcc_libdir}" >/dev/null)
        ...
        if [[ "${host_goos}" == windows || "${host_arch}" == arm64 ]]; then
            if [[ "${host_goos}" == windows ]]; then
                make -C "${build_dir}" CC="${host_make_cc}" CFLAGS="${host_cflags}" LDFLAGS="${host_ldflags}" cproc-qbe
            else
    ...
    install -m 0755 "${build_dir}/${target}${binary_suffix}" "${stage}/bin/${target}${binary_suffix}"
}
...
bootstrap_windows_guest_tools() {
    [[ "${host_goos}" == windows ]] || return 0
    bootstrap_bin="${work_dir}/bootstrap/bin"
    mkdir -p "${bootstrap_bin}"
    env CGO_ENABLED=0 GOOS=linux GOARCH=amd64 "${go_command}" build -trimpath -tags ie64 -o "${bootstrap_bin}/ie64asm" "${root_dir}/assembler"
    env CGO_ENABLED=0 GOOS=linux GOARCH=amd64 "${go_command}" build -trimpath -o "${bootstrap_bin}/ie64-cproc" "${root_dir}/cmd/ie64-cproc"
    env CGO_ENABLED=0 GOOS=linux GOARCH=amd64 "${go_command}" build -trimpath -o "${bootstrap_bin}/ie64-ar" "${root_dir}/cmd/ie64-ar"
    env CGO_ENABLED=0 GOOS=linux GOARCH=amd64 "${go_command}" build -trimpath -o "${bootstrap_bin}/ie64ld" "${root_dir}/cmd/ie64ld"

    bootstrap_qbe="${work_dir}/bootstrap-qbe"
    cp -a "${qbe_dir}" "${bootstrap_qbe}"
    make -C "${bootstrap_qbe}" clean-gen >/dev/null
    make -C "${bootstrap_qbe}" CC=cc qbe >/dev/null
    install -m 0755 "${bootstrap_qbe}/qbe" "${bootstrap_bin}/qbe"

    bootstrap_cproc="${work_dir}/bootstrap-cproc"
    cp -a "${cproc_dir}" "${bootstrap_cproc}"
    make -C "${bootstrap_cproc}" clean >/dev/null
    rm -f "${bootstrap_cproc}/config.h" "${bootstrap_cproc}/config.mk"
    (cd "${bootstrap_cproc}" && CC=cc ./configure --host=x86_64-linux-gnu --target=x86_64-linux-gnu >/dev/null)
    make -C "${bootstrap_cproc}" CC=cc all >/dev/null
    install -m 0755 "${bootstrap_cproc}/cproc-qbe" "${bootstrap_bin}/cproc-qbe"
}

run_guest_tool() {
    if [[ "${host_goos}" == windows ]]; then
        "${bootstrap_bin}/$(basename "$1")" "${@:2}"
    else
        run_host_tool "$@"
    fi
}
...
"${go_build[@]}" -trimpath -o "${stage}/bin/ie32asm${binary_suffix}" "${root_dir}/assembler/ie32asm.go"
"${go_build[@]}" -trimpath -o "${stage}/bin/ie32to64${binary_suffix}" "${root_dir}/cmd/ie32to64"
"${go_build[@]}" -trimpath -tags ie64 -o "${stage}/bin/ie64asm${binary_suffix}" "${root_dir}/assembler"
"${go_build[@]}" -trimpath -tags ie64dis -o "${stage}/bin/ie64dis${binary_suffix}" "${root_dir}/assembler"
"${go_build[@]}" -trimpath -o "${stage}/bin/ie64-cproc${binary_suffix}" "${root_dir}/cmd/ie64-cproc"
"${go_build[@]}" -trimpath -o "${stage}/bin/ie64ld${binary_suffix}" "${root_dir}/cmd/ie64ld"
"${go_build[@]}" -trimpath -o "${stage}/bin/ie64-ar${binary_suffix}" "${root_dir}/cmd/ie64-ar"
"${go_build[@]}" -trimpath -o "${stage}/bin/ie64-ranlib${binary_suffix}" "${root_dir}/cmd/ie64-ranlib"
build_external_tool qbe "${qbe_dir}" qbe
build_external_tool cproc "${cproc_dir}" cproc-qbe
bootstrap_windows_guest_tools
for executable in "${stage}/bin"/*; do
    if [[ "${host_goos}" == windows ]]; then
        file "${executable}" | grep -Eq 'PE32\+ executable.*(x86-64|x86_64)' || fail "expected Windows x64 PE: ${executable}"
                # Inspect imports with objdump -p and allow only Windows system DLLs.
                imports="$(${host_objdump} -p "${executable}" | awk '/DLL Name:/ { print toupper($3) }')"
                if [[ -n "${imports}" ]] && printf '%s\n' "${imports}" | grep -Evq '^(ADVAPI32|API-MS-WIN-[A-Z0-9-]+|BCRYPT|COMBASE|CRYPT32|GDI32|KERNEL32|MSVCRT|NTDLL|OLE32|OLEAUT32|RPCRT4|SECHOST|SHELL32|USER32|VERSION|WINHTTP|WINMM|WS2_32)\.DLL$'; then
                    fail "Windows executable imports a non-system DLL: ${executable}"
                fi
    elif [[ "${host_arch}" == arm64 ]]; then
...
run_guest_tool "${stage}/bin/ie64asm" -c -Werror -o "${stage}/lib/ie64-unknown-none/crt0.o" "${root_dir}/sdk/lib/ie64-cproc/crt0.s"
...
run_guest_tool "${stage}/bin/ie64-cproc" --sysroot "${root_dir}/sdk" -c -o "${work_dir}/libatomic.o" "${root_dir}/sdk/lib/ie64-cproc/libatomic.c"
run_guest_tool "${stage}/bin/ie64-ar" rcs "${stage}/lib/ie64-unknown-none/libatomic.a" "${work_dir}/libatomic.o"
restore_nested_qemu_tools
...
archive="${root_dir}/dist/${package_name}.${archive_format}"
rm -f "${archive}" "${archive}.sha256"
if [[ "${archive_format}" == zip ]]; then
    (cd "${work_dir}" && find "${package_name}" -print | LC_ALL=C sort | zip -X -q -@ "${archive}")
else
...
if [[ "${archive_format}" == zip ]]; then
    (cd "${work_dir}" && find "${package_name}" -print | LC_ALL=C sort | zip -X -q -@ "${work_dir}/repro.zip")
    cmp "${archive}" "${work_dir}/repro.zip"
else
...
cmp "${archive}" "${root_dir}/dist/${package_name}.${archive_format}"
