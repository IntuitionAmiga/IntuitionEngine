...
- Go 1.27 native JIT calls use `runtime.cgocall`. Linux VM and benchmark builds enable cgo, while Windows and macOS release builds retain their existing `CGO_ENABLED=0` configurations.
...
- **AROS Audio DMA**: AROS Paula-style DMA shim routed through SoundChip FLEX DAC channels.
...
#### AROS Support
- AROS boot with full Workbench desktop (12 tasks, 27.6MB RAM plus 64MB direct VRAM).
...
- Memory layout: 5.6MB chip + 22MB fast, direct VRAM at 0x1E00000-0x5DFFFFF (64MB).
