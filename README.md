...
| Key | Action |
|-----|--------|
| `F7` | Cycle the CRT presentation filter through flat, curved, and off. CRT is off by default. The key also reaches the guest when no host overlay is consuming keyboard input. |
...
- M15.1 source layout status: `sdk/intuitionos/iexec/iexec.s` remains the kernel assembly entrypoint; `sdk/intuitionos/iexec/runtime_builder.s` and `sdk/intuitionos/iexec/cmd/` contain the hostfs runtime artefacts and command sources.
