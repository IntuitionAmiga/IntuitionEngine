...
func (e *MODEngine) updateDAC() {
	...
	for i := range modChannels {
		...
		if !mc.active || mc.sample == nil || len(mc.sample.Data) == 0 || mc.period == 0 {
			e.writeChannel(ch, FLEX_OFF_DAC, 0)
			continue
		}

		// Read sample byte
		sampleByte, active := mc.ReadSample()
		if !active {
			e.writeChannel(ch, FLEX_OFF_DAC, 0)
			continue
		}
		...
		scaled := int(sampleByte) * mc.volume / 64

		// Apply Amiga filter if model is set
		if e.filterModel != 0 {
			scaledF := float32(scaled) / 128.0
			scaledF = e.applyAmigaFilter(i, scaledF)
		...
		e.writeChannel(ch, FLEX_OFF_DAC, uint32(byte(int8(scaled))))
	}
}
