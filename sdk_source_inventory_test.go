...
func sdkArchitectureFactsFromSource(t *testing.T) []sdkSourceFact {
	...
	for _, row := range []struct {
		...
		{"Guest-Advanced is the default CRT profile but CRT presentation is initially off. F7 cycles off, flat, curved, and off while continuing to reach the guest.", "`video_backend_ebiten.go` `NewEbitenOutput`/`Update`/`handleKeyboardInput`, `video_crt_filter_ebiten.go` mode ordering, and `video_backend_ebiten_status_test.go`"},
		...
		{"The Linux x86-64 and ARM64 Host SDKs package ie32asm, ie32to64, ie64asm, ie64dis, ie64-cproc, ie64ld, ie64-ar, ie64-ranlib, QBE, cproc-qbe, the IE64 runtime and libraries, public assembly includes, intuitionengine.h, and user documentation.", "`scripts/dist-host-sdk-linux-amd64.sh` and `scripts/dist-host-sdk-linux-arm64.sh` staged tools, libraries, includes, and documentation; `host_sdk_test.go` public-header contract"},
		{"The x64 live image stages both Host SDK archives and their SHA-256 files under SDK/Toolchains; the former per-platform SDK/Tools tree and standalone IE64 toolchain archive are not staged.", "`build_x64_ie_img.sh` `stage_share_payload`/`verify_staged_share_payload`, `x64_live_test.go` Host SDK payload assertions"},
