...
func sdkArchitectureFactsFromSource(t *testing.T) []sdkSourceFact {
	...
	for _, row := range []struct {
		...
		{"The AROS host socket block at 0xF2500-0xF257F uses 96-byte big-endian request descriptors, strict guest-memory helper copies, 64 KiB send/recv caps, 128-byte sockaddr caps, guest-visible descriptor handles, WaitSelect fd_set translation, and ENOSYS fail-closed behaviour when disabled.", "`aros_host_socket_constants.go` command/register constants, `aros_host_socket.go` dispatch/copy limits, `aros_host_socket_unix.go` descriptor table and WaitSelect implementation, `main.go` socket MMIO registration"},
