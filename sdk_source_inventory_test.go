...
func sdkArchitectureFactsFromSource(t *testing.T) []sdkSourceFact {
	...
	for _, row := range []struct {
		...
		{"The Linux x86-64, Linux ARM64 and Windows x86-64 Host SDKs package ie32asm, ie32to64, ie64asm, ie64dis, ie64-cproc, ie64ld, ie64-ar, ie64-ranlib, QBE, cproc-qbe, the IE64 runtime and libraries, public assembly includes, intuitionengine.h, and user documentation.", "`scripts/dist-host-sdk-linux-amd64.sh`, its Linux ARM64 wrapper and its Windows x86-64 wrapper staged tools, libraries, includes, and documentation; `host_sdk_test.go` public-header contract"},
		{"The x64 live image stages all three Host SDK packages and their SHA-256 files under SDK/Toolchains; the former per-platform SDK/Tools tree and standalone IE64 toolchain archive are not staged.", "`build_x64_ie_img.sh` `stage_share_payload`/`verify_staged_share_payload`, `x64_live_test.go` Host SDK payload assertions"},
