...
230 REM Block steps
240 BX=CA*8: BY=SA*8
250 EX=-SA*16: EY=CA*16
260 REM Starting U,V for top-left block centre
270 SU=128-40*BX-15*EX: SV=128-40*BY-15*EY
280 RU=SU: RV=SV
300 REM === RENDER 80x30 BLOCKS (8x16 pixels) TO BACK BUFFER ===
310 FOR Y=0 TO 29
320 U=RU: V=RV: P=BB+Y*40960
330 FOR X=0 TO 79
340 TU=INT(U) AND 255: TV=INT(V) AND 255
350 C=PEEK(TB+TV*1024+TU*4)
360 BLIT FILL P, 8, 16, C, 2560
