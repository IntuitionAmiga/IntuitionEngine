...
func TestJIT_AMD64_IE64Store_AboveMemSize_BailsToInterpreter(t *testing.T) {
	...
	if r.ctx.NeedHelper != HELPER_STORE {
		t.Fatalf("NeedHelper = %d, want HELPER_STORE (clean helper-exit above MemSize)", r.ctx.NeedHelper)
	}
}
...
func TestJIT_AMD64_IE64FLoad_NearEndOfMemory_Bails(t *testing.T) {
	...
	if r.ctx.NeedHelper != HELPER_FLOAD {
		t.Fatalf("FLOAD addr=MemSize-3: NeedHelper = %d, want HELPER_FLOAD (size-aware helper-exit)", r.ctx.NeedHelper)
	}
}
