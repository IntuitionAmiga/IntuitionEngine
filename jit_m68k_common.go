...
// m68kBuildJITIOPageBitmap snapshots the MachineBus I/O page bitmap (plus
// CPU-specific always-I/O pages) into cpu.m68kJitIOPageBitmap for the native
// backends' inline access guards. Shared by the amd64 and arm64 dispatchers
// (M68K JIT parity plan milestone 3): every native backend must route
// MMIO-page accesses through the interpreter, and the coprocessor shared
// window must skip the inline big-endian swap fast path.
func (cpu *M68KCPU) m68kBuildJITIOPageBitmap() {
