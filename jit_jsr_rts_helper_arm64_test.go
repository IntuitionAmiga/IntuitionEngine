// jit_jsr_rts_helper_arm64_test.go — Phase 5 JSR/RTS/JSR_IND helper-exit tests (ARM64).
...
func TestJIT_ARM64_JSR_HighSP_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
...
func TestJIT_ARM64_JSR_MMUEnabled_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
...
func TestJIT_ARM64_JSR_LowSP_NoHelper(t *testing.T) {
	r := newJITTestRig(t)
	r.cpu.regs[31] = STACK_START
...
func TestJIT_ARM64_RTS_HighSP_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
	const highSP uint64 = 0x0000_0001_0000_8000
	r.cpu.regs[31] = highSP
	r.ctx.NeedHelper = HELPER_NONE
...
func TestJIT_ARM64_RTS_MMUEnabled_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
	r.cpu.regs[31] = STACK_START
	r.ctx.NeedHelper = HELPER_NONE
	r.ctx.MMUEnabled = 1
...
func TestJIT_ARM64_RTS_LowSP_NoHelper(t *testing.T) {
	r := newJITTestRig(t)
...
func TestJIT_ARM64_JSR_IND_HighSP_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
	const highSP uint64 = 0x0000_0001_0000_8000
...
// TestJIT_ARM64_JSR_IND_R31_HighSP_UsesDecrementedSP — see AMD64 twin. The
// interpreter decrements SP before resolving rs+imm, so rs==R31 targets must
// be based on SP-8.
func TestJIT_ARM64_JSR_IND_R31_HighSP_UsesDecrementedSP(t *testing.T) {
	r := newJITTestRig(t)
	const highSP uint64 = 0x0000_0001_0000_8000
	r.cpu.regs[31] = highSP
	r.ctx.NeedHelper = HELPER_NONE
...
func TestJIT_ARM64_JSR_IND_MMUEnabled_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
...
func TestJIT_ARM64_JSR_HighSP_HelperEndToEnd(t *testing.T) {
...
func TestJIT_ARM64_RTS_HighSP_HelperEndToEnd(t *testing.T) {
	const highAddr uint64 = 0x0000_0001_0000_8000
	const retAddr uint64 = PROG_START + IE64_INSTR_SIZE
...
func TestJIT_ARM64_JSR_IND_HighSP_HelperEndToEnd(t *testing.T) {
	const highAddr uint64 = 0x0000_0001_0000_8000
