...
const (
	...
	am6502IndX           // (nn,X)
	am6502IndY           // (nn),Y
	am6502AbsIndX        // (nnnn,X)
	am6502Rel            // relative
	am6502ZpRel          // nn, relative
)
...
func disassemble6502(readMem func(addr uint64, size int) []byte, addr uint64, count int) []DisassembledLine {
	...
	for range count {
		...
		// Branch annotation
		switch info.mode {
		case am6502Rel, am6502ZpRel:
			line.IsBranch = true
			if info.mode == am6502ZpRel && size >= 3 {
				line.BranchTarget = uint64(uint16(addr) + 3 + uint16(int8(data[2])))
			} else if size >= 2 {
