// jit_push_pop_helper_amd64_test.go — Phase 5 cycle 5.8 AMD64 PUSH/POP helper-exit tests.
...
func TestJIT_AMD64_PUSH_HighSP_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
...
func TestJIT_AMD64_POP_HighSP_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
	const highSP uint64 = 0x0000_0001_0000_8000
	r.cpu.regs[31] = highSP
	r.ctx.NeedHelper = HELPER_NONE
...
func TestJIT_AMD64_PUSH_MMUEnabled_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
...
func TestJIT_AMD64_POP_MMUEnabled_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
	r.cpu.regs[31] = STACK_START
	r.ctx.NeedHelper = HELPER_NONE
	r.ctx.MMUEnabled = 1
...
// TestJIT_AMD64_PUSH_LowSP_NoHelper — fast path: low SP, MMU off → direct
// stack write, no helper request.
func TestJIT_AMD64_PUSH_LowSP_NoHelper(t *testing.T) {
	r := newJITTestRig(t)
...
// TestJIT_AMD64_POP_LowSP_NoHelper — fast path POP.
func TestJIT_AMD64_POP_LowSP_NoHelper(t *testing.T) {
	r := newJITTestRig(t)
...
// TestJIT_AMD64_PUSH_R31_HighSP_StoresDecrementedSP — PUSH R31 must push
// SP-8 (interpreter decrements SP before reading R31). The helper path
// captured the pre-decrement SP; this pins the corrected HelperVal.
func TestJIT_AMD64_PUSH_R31_HighSP_StoresDecrementedSP(t *testing.T) {
	r := newJITTestRig(t)
	const highSP uint64 = 0x0000_0001_0000_8000
	r.cpu.regs[31] = highSP
	r.ctx.NeedHelper = HELPER_NONE
...
// TestJIT_AMD64_PUSH_R31_HighSP_EndToEnd — full round-trip: PUSH R31 at
// high SP, then read the backing slot. Interpreter stores SP-8.
func TestJIT_AMD64_PUSH_R31_HighSP_EndToEnd(t *testing.T) {
	const highAddr uint64 = 0x0000_0001_0000_8000

	_, backing := runIE64HighBackingTest(t,
		func(cpu *CPU64) {
...
func TestJIT_AMD64_PUSH_HighSP_HelperEndToEnd(t *testing.T) {
	const highAddr uint64 = 0x0000_0001_0000_8000
...
func TestJIT_AMD64_POP_HighSP_HelperEndToEnd(t *testing.T) {
	const highAddr uint64 = 0x0000_0001_0000_8000
