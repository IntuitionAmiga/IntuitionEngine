...
Mailbox RAM starts at `MAILBOX_BASE = 0x790000` and has `MAILBOX_SIZE = 0x3000` bytes, ending at `0x792FFF`. There are `COPROC_RING_COUNT = 12` rings, each with `RING_CAPACITY = 16` and `RING_STRIDE = 0x400`. The ring index is `cpuTypeIndex*2 + instance`, where `cpuTypeIndex` (from `cpuTypeToIndex`) orders the types `IE32`, `6502`, `M68K`, `Z80`, `x86`, `IE64`; this axis is documented separately from the `EXEC_TYPE_*` numeric values. Instance 0 of each type takes the even ring, instance 1 the following odd ring. The three single-instance types (IE32, 6502, Z80) leave their odd rings reserved but unused, keeping the addressing purely arithmetic. The `0x400` stride exceeds a ring's true content size of `0x308` bytes, so no ring's slot-15 response overflows into the next ring's header. The ring uses the conventional `nextHead == tail` full test, so at most 15 descriptors are usable at once; one slot is reserved to distinguish full from empty.

At startup a conforming worker echoes the mailbox layout version (`COPROC_LAYOUT_VERSION`, byte offset `0x04` in its ring header) so the manager routes it work; a stale image built for a different layout never reaches that address and START fails with `COPROC_ERR_STALE_WORKER`.
