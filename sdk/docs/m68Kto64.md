...
| `-size .l\|.q` | Reserved — currently no-op. Sizeless m68k mnemonics resolve via `SizeBytes("")` in `emit.go` which hard-codes `.w` (2 bytes) per m68k convention; flag is parsed but never read |
...
| aux scratch | r19 (ScrAux) | used by DIVU.W/DIVS.W (quotient holder while remainder is computed), `emitShiftCPre` `NOT(dst XOR src)` temp, and `storeValue` partial-update merge intermediate for `.b`/`.w` writes into Dn |
...
§4.6: "All `d*` mnemonics require even-numbered FP operands in assembly. Writing a
...
| `JSR target` (label / abs / PC-rel) | push 32-bit return label (auto-generated `__m68kto64_jsr_ret_N`); `bra <label>` |
...
| `(d8,PC,Xn[.size][*scale])` | `la ScrEA, label`; sext/move Xn into a temp; if scale > 1, shift the temp by `log2(scale)`; `add.l ScrEA, ScrEA, <temp>`; if d8 ≠ 0, `add.l ScrEA, ScrEA, #d8` |
...
| `#imm` (FP) | `fmovecr fd, #idx` if it matches an IE64 ROM entry; else allocate `dc.s`/`dc.d` in `__m68kto64_fp_const_pool` then `la r16, label; fload/dload f(2n), (r16)` (FP const-pool path uses ScrEA = r16, not ScrV1) |
...
the canonical pre-modify source field; ASL V-on-sign-change is approximated as V=0;
DIVU.W/DIVS.W N reflects bit-31 of the packed `(rem<<16)|quo` (sign of remainder),
not bit-15 of the quotient. Standalone Bcc
...
| `LSL` / `LSR` / `ASL` / `ASR` / `ROL` / `ROR` | direct ops; full N/Z, C from last bit shifted out, V=0 (ASL V-on-sign-change not modeled — approximation), X tracks C for LSL/LSR/ASL/ASR (ROL/ROR leave X unchanged) | ⚠️ ASL V approximate |
...
| `ABCD` / `SBCD` / `NBCD` | BCD lowering, both Dn,Dn and -(Ay),-(Ax) forms; X-flag chain-in carry honored | ✅ |
...
| `BRA` / `BSR` / `JMP` / `JSR` / `RTS` / `RTR` | direct/emulated 32-bit return on r30; RTR pops a 16-bit CCR + 32-bit PC frame and unpacks CCR bits (X=4, N=3, Z=2, V=1, C=0) into shadow regs r24–r28 | ✅ |
...
`TRAP #n` → `syscall #n` (`assembler/ie64asm.go:3897` — `asmSYSCALL`). Locked syscall # range
...
directly. The `.div_okN` label is generated via the same `labelSeq` counter as
JSR/BSR return labels.
...
- **Shadow-flag gaps.** `MULU.L`/`MULS.L`/`DIVU.L`/`DIVS.L` (pair forms) and
  ...
  not modeled. ASL V-on-sign-change-during-shift is approximated as V=0. Downstream
  Bcc consumers against these producers must not rely on m68k-exact CCR semantics.
...
`sf`, `st`, `seq`, `sne` (see `fpFuseSupported` in
...
fused emit additionally executes `fmovsr ScrV1; or.q ScrV1, ScrV1, #1;
fmovsc ScrV1` to set hardware FPSR bit 3; the other 10 fused cc kinds do
not touch FPSR sticky bits. An intervening `fmove.l Dn,fpsr` or `fmovsc`
