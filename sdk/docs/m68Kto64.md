...
| `CHK2` | unsupported (no handler; lexer recognises but `-Werror-unknown-mnemonic` errors) | ❌ |
| `TRAPV` | guarded `syscall #18` against shadow V | ✅ |
| `TRAPcc` (68020 integer-cc form) | unsupported (no handler; mnemonics `trapeq`/`trapne`/… not in lexer set) | ❌ |
...
| `#17` | CHK fail (m68k vector 6, relocated) | bounds-test bltz/bgt fall-through (CHK2 unsupported — see §8) |
| `#18` | TRAPV (m68k vector 7, relocated) | guarded on shadow V |
...
- **CHK2 / integer TRAPcc.** Not implemented. `CHK2` and the 68020 integer-cc `TRAPcc`
  family (`TRAPEQ`/`TRAPNE`/…) reach the unknown-mnemonic path and error under the
  default `-Werror-unknown-mnemonic=true`. Use plain `CHK` and `TRAPV` (or rewrite the
  cc guard with an explicit Bcc + `TRAP #n`).
...
- **ADDX / SUBX / NEGX / ROXL / ROXR.** Unsupported (X-flag chain-in arithmetic). The
  ShadowX register (r28) is maintained on producers but no consumer lowering is wired.
- **RTM / MOVES / CALLM / RETM.** Unsupported supervisor/68020-loose-end ops.
