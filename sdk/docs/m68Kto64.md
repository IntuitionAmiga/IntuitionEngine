...
hand-rolled decimal conversion when needed. FSAVE and FRESTORE are fully
lowered to a transpiler-private 80-byte state frame (see §8.FP and §11):
symmetric save/restore round-trips perfectly, and FRESTORE falls back to
a null-frame clear when the format magic does not match — covering both
in-process context switches and the m68881 cold-boot `clr.l -(sp);
frestore (sp)+` idiom. Programs that introspect the m68881 frame format
byte at offset 0 will read an IE64 double bit pattern instead and must
be rewritten if that bit-level introspection matters.
