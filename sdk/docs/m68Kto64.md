...
| `RTE` | forward to `RTS` (SR pop dropped; no IE64 supervisor model) | ⚠️ SR pop dropped |
| `ILLEGAL` | `syscall #19` (m68k vector 4) | ✅ |
| `STOP` | stripped + diagnostic (privileged; no IE64 supervisor halt) | ⚠️ stripped |
| `RESET` | stripped + diagnostic (privileged peripheral reset; no IE64 analog) | ⚠️ stripped |
| `MOVEC` | stripped + diagnostic comment `; m68kto64: stripped movec ...` | ⚠️ |
| `TAS <ea>` | non-atomic byte RMW; N/Z from pre-op byte, V/C := 0, bit 7 set in dst | ⚠️ non-atomic |
...
- **BSET / BCLR / BCHG.** Lowered (`emitBitRmw` in `cmd/m68kto64/converter.go`)
  ...
  N/V/C unaffected.
- **Privileged / system ops.** `ILLEGAL` is lowered as `syscall #19`
  (m68k vector 4, relocated — see §11). `RTE` forwards to `RTS` with the SR pop
  dropped and a ⚠️ diagnostic (no IE64 supervisor model). `STOP` and `RESET`
  are stripped with diagnostics — neither has an IE64 analog. `MOVEC` likewise
  strips. Per Strict-mode policy, the privileged strips do not error under
  `-strict` (no user-mode-reachable semantic loss).
- **TAS / EXG / CMPM.** Lowered: `emitTas` / `emitExg` / `emitCmpm` in
  `cmd/m68kto64/converter.go`. `TAS` is a non-atomic byte RMW (mirrors
  CAS / CAS2 atomicity caveat); EXG is a 3-instruction register exchange via
  ScrV1; CMPM postinc-loads both operands and feeds the existing sub-shadow
  path. The previously orphaned `CMPM` shadow case in `ccr_shadow.go` is now
  reachable through the main dispatch.
