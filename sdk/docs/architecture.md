...
GPU-side compatibility-format conversion is opt-in with `IE_VIDEO_GPU_CONVERT=1` and needs a shader-capable Ebiten backend, so headless builds always convert on the CPU whatever the switch says. The CPU converter remains the canonical oracle. The scheme uploads the indexed frame as a single-channel texture and the palette as a 256 by 1 texture, and looks the palette up in the fragment shader. Only the format-independent parts (palette texture layout, the shader's index arithmetic and its pure-Go mirror, and the selection policy) are in place; the shader is not yet wired into the live pixel path, which awaits the render-and-readback differential gate on real hardware.
