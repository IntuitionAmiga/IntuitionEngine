...
Conversion is not yet wired into the live pixel path, which carries RGBA throughout, so the switch currently selects nothing at runtime. The planar formats (VGA, ULA, TED, ANTIC) are not implemented either, since their per-scanline register state is not the fixed index arithmetic a fragment shader can mirror.
