...
| `$03`  | Memory indirect  | Reads use the word at `operand`; stores and memory updates use that word as the final destination address |
| `$04`  | Direct           | Reads use the word at `operand`; stores and memory updates write directly to `operand` |
...
Mode `$03` needs special care because its meaning depends on whether the
instruction reads or writes. A load with operand `$2000` reads the word at
`$2000`. A store with the same mode and operand first reads a pointer from
`$2000`, then writes through that pointer. The mode has no mnemonic source
form, so enter its mode byte through IE Mon when you need it.

This example places the pointer `$3000` at `$2000`. The indirect `STA` writes
`$2A` through that pointer. The indirect `LDX` reads the pointer word itself,
so the following direct `STX` writes `$3000` to `$4000`.

```
(ie32)> w 2000 00 30 00 00
(ie32)> w 1200 20 00 00 00 2A 00 00 00 24 00 03 00 00 20 00 00
(ie32)> w 1210 21 01 03 00 00 20 00 00 25 01 04 00 00 40 00 00
(ie32)> w 1220 FF 00 00 00 00 00 00 00
(ie32)> r pc 1200
(ie32)> d 1200 #5
> 001200: 20 00 00 00 2A 00 00 00  LDA A, #$0000002A
  001208: 24 00 03 00 00 20 00 00  STA A, [M]$00002000
  001210: 21 01 03 00 00 20 00 00  LDX X, [M]$00002000
  001218: 25 01 04 00 00 40 00 00  STX X, M:$00004000
  001220: FF 00 00 00 00 00 00 00  HALT
(ie32)> g
(ie32)> m 3000 1
00003000: 2A 00 00 00 00 00 00 00  00 00 00 00 00 00 00 00  *...............
(ie32)> m 4000 1
00004000: 00 30 00 00 00 00 00 00  00 00 00 00 00 00 00 00  .0..............
```

The first dump proves that the store followed the pointer. The second proves
that the load stopped after reading the word at `$2000`; it did not perform a
second read from `$3000`.
