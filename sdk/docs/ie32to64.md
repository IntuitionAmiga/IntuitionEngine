...
**Scratch registers**: r17 is the primary scratch register for address loads (`la r17, addr`) and DIV/MOD guard materialization. r18-r20 are used for multi-step expansions like `INC @addr`.
...
| `STORE A, X` | **ERROR** | IE32 register STORE has broken CPU semantics; converter refuses it |
...
IE32 halts when `DIV` or `MOD` sees a zero divisor. IE64 writes zero and continues. By default, the converter preserves IE32 behavior by guarding any divisor that is not a proven non-zero literal.
...
Symbolic immediates and bare expressions are materialized into r17 before the guard because the converter does not evaluate expressions:
...
Use `-no-div-guard` only when you deliberately want legacy converter output and have proven zero divisors cannot occur.
...
For register or memory cycle counts, convert the source to an explicit busy-wait loop before running `ie32to64`.
...
`incbin "relative/path"` is emitted verbatim. If `-o` writes the converted file to a different directory than the source, relative include paths may no longer resolve. Prefer writing output next to the source or pre-rewrite paths.
...
4. **Unknown directives and mnemonics produce hard errors** - typos and non-standard syntax
