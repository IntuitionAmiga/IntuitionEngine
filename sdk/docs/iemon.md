...
Whole-machine snapshots cover all monitor-registered CPUs, shared bus RAM, sparse IE64 backing memory, and registered versioned device blobs. The history stores full sparse checkpoints plus sparse deltas anchored to a retained checkpoint; `rg` and `rt` materialise deltas before replay or restore. By default a delta is formed by scanning all of guest RAM and diffing it against the previous capture. The monitor arms a bus page-dirty cursor automatically the first time reverse history is recorded, then drives each delta from it, copying only the pages the guest wrote since the last capture; full checkpoints still take the complete scan and rebaseline the cursor, and `IE_MON_EPOCH_HISTORY=0` forces the legacy full-scan path. The reconstructed state is identical either way, so `rg` and `rt` behave the same with the switch on or off. The production monitor registers the main video chip, sound chip, terminal MMIO, command-style host helpers (file I/O, media loader, program executor, and coprocessor manager), compatibility audio/video engines (PSG/AY, SN76489, SID/SID2/SID3, TED audio, POKEY, VGA, ULA, TED video, ANTIC/GTIA, and Voodoo), and optional host bridges when present. Additional devices join the same contract through `RegisterSnapshotDevice`. Timeline replay depends on deterministic device snapshot and restore behaviour.
