...
| Fast MMIO polling | Adopted on AMD64 where its pattern matches | The native dispatcher validates the live adapter, executes the recognised poll through it, and yields after an unchanged bounded batch. |
...
### MMIO Poll Scheduling

On AMD64, the dispatcher recognises an absolute `LD A,(nn)`, `AND mask`, and
backward `JR Z` or `JR NZ` polling loop only when the translated address is
live mapped I/O. Each read still goes through `Z80BusAdapter`, preserving the
device access and flag behaviour of the interpreter.

If a complete bounded batch observes no state change, the dispatcher yields the
host scheduler before retrying the unchanged guest loop. This lets the video
refresh goroutine publish a VBlank edge instead of being starved by polling.
Linux ARM64 retains the canonical helper path for this optimisation.

The browser wasm dispatcher recognises the same Z80 instruction shape in its
own loop. It reads through `Z80BusAdapter`, applies the `AND` flags including
clearing carry, charges 32 cycles for a taken backward `JR` and 27 when it
falls through, and preserves the `R` register increment. It then parks between
unchanged iterations. The park gives the browser event loop time to update
VBlank and other device state before the next guest read.
