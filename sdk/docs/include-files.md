...
This M16 surface is intentionally narrow. `iexec.inc` documents the shipped ABI constants; the larger manifest/schema/state-machine discussion lives in `sdk/docs/IntuitionOS/IExec.md` and `sdk/docs/IntuitionOS/M16-plan.md`. M16.2 extends the same internal lifecycle model to handlers, devices, and resources with `MODCLASS_HANDLER`, `MODCLASS_DEVICE`, `MODCLASS_RESOURCE`, plus trusted-internal `SYS_ADD_HANDLER`, `SYS_ADD_DEVICE`, and `SYS_ADD_RESOURCE` aliases. Public non-library acquisition wrappers remain M16.2.1 work. The M14.2 `ET_EXEC` loader boundary remains unchanged. PIE enforcement is reserved for M16.3 and relocation/ASLR for M16.4.
