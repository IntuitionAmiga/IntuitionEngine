...
IntuitionOS is the operating environment built around IExec. Its user-facing shape is deliberately Amiga-like: `exec.library` provides tasks, signals, message ports, library acquisition, and memory allocation; `dos.library` provides assign-based path resolution and command launch; handlers, devices, resources, and libraries are named services reached through ports and small fixed message protocols. The difference is that the old Amiga convenience of sharing raw pointers in one flat privileged address space is not part of the contract. Cross-task data transfer uses explicit shared-memory capabilities, services run as protected user tasks, and the kernel owns only the mechanisms that require privileged CPU or MMU state.

This document has two purposes. Sections 1-11 describe the current IntuitionOS and IExec contract for developers. Section 12 records milestone history and regression context; it is intentionally changelog-like and should not be read as the current API surface when it conflicts with the earlier current-contract sections.

**Current System Shape**

- `exec.library` is the protected nucleus. It owns task lifecycle, scheduling, signals, message ports, shared-memory handles, page-table changes, trap/fault handling, and protected-module lifecycle.
- `dos.library`, `console.handler`, `input.device`, `hardware.resource`, `graphics.library`, and `intuition.library` are protected service tasks. They are not linked into a flat kernel address space.
- Applications and commands are runtime ELFs using the current `ET_DYN` / `PT_NOTE` contract with IOSM metadata, local `R_IE64_RELATIVE64` relocation records where needed, W^X mappings, and ASLR.
- Device access is mediated by service protocols and grant-gated MMIO. A normal application does not receive ambient access to the chip register page or VRAM.
- The system presents Amiga-style names and workflows (`C:`, `S:`, `LIBS:`, `DEVS:`, `RESOURCES:`, `OpenLibrary`, `FindPort`, `PutMsg`, `WaitPort`, `ReplyMsg`) while replacing flat pointers, global supervisor state, and ad hoc launch shortcuts with explicit capabilities and validated loader metadata.

**Security Model**

IExec's security boundary is the IE64 MMU plus a deliberately small syscall surface. The kernel does not rely on convention to keep tasks honest:

- each task has its own page table; user mappings are `PTE_U`, supervisor mappings are not
- executable user pages are never writable, writable user pages are never executable, and dynamic/shared user memory is non-executable
- supervisor user-memory access is blocked by `SKAC` except inside explicit `SUAEN` / `SUADIS` copy windows; supervisor instruction fetch from user pages is blocked by `SKEF`
- kernel copies from and to user space go through permission-aware helper paths rather than raw dereferences
- shared memory is mapped by opaque share handle plus explicit `MAPF_READ` / `MAPF_WRITE` permissions; stale handles and wrong permissions fail rather than degrading to raw pointer sharing
- task exit, fault teardown, module crash cleanup, shared-memory reference cleanup, and grant revocation are kernel-owned cleanup paths, not best-effort user code
- bootstrap HostFS path resolution rejects symlink components with per-component `NOFOLLOW` and case-normalises paths before access
- runtime ELF loading rejects dynamic linking, external-symbol relocation, text relocation, unsupported relocation types, malformed IOSM metadata, and untrusted protected-module provenance

The security goal is not POSIX users, groups, or a multi-tenant policy model. It is a retro-computing OS where classic Exec-style programming remains ergonomic, but memory corruption in one task should not silently become kernel corruption, cross-task pointer access, or ambient hardware access.

**Amiga-Like Without Legacy Cruft**

The preserved Amiga ideas are the useful ones: small services, named ports, request/reply IPC, library acquisition by name, assign-based paths, resident modules, and a shell that runs real commands rather than shell builtins. The removed legacy assumptions are the unsafe ones:

- no `SysBase` pointer in user memory
- no public TCB pointer or `ThisTask` structure exposed to applications
- no raw message payload pointers between unrelated tasks
- no user-mode interrupt masking or scheduler disable primitives
- no flat-image application launch path
- no ambient `MapVRAM` or broad `MapIO` allowlist for applications
- no dynamic linking boundary masquerading as a library API; services communicate through explicit ports/messages

**Current IExec Capabilities**

- Preemptive round-robin scheduling across up to 255 live tasks (CreateTask/ExitTask with slot reuse). M5 capped this at 8 (per-task `USER_DYN_STRIDE` saturated the 32 MiB VA at 8 tasks); M12 globalised the dynamic VA window and bumped the cap to 16; M12.6 Phase D bumped it to 32; **M13 phase 4 expands the remaining fixed task-state tables to the current 8-bit internal-slot ABI ceiling (255, with `0xFF` reserved as a sentinel in a few kernel fields)**. Public task IDs are independent monotonic `u32`s.
...
- Context switching (save/restore PC, USP, PTBR, and the timer-preemption GPR frame saved on the user stack)
...
- AmigaOS-style `OpenLibrary` for library discovery - **M11.5**: new source should call `SYS_FIND_PORT` directly; slot 36 (`SYS_OPEN_LIBRARY`) is retained as a one-instruction binary-compat redirect to `.do_find_port` so any pre-M11.5 IE64 binary still links. See § 5.11 "Exec Boundary".
- Grant-gated I/O page mapping via `MapIO` (`REGION_IO`) for trusted services and bootstrap-granted handlers
...
- **M15.6 HostFS hardening**: bootstrap HostFS resolution is per-component `NOFOLLOW` and case-normalised. Any symlink component in a bootstrap path is rejected instead of traversed, and case variants collapse to one canonical host path. See [HostFS.md](HostFS.md).
...
- Direct filesystem policy in the nucleus. DOS path resolution, the RAM-backed store, the host-backed `SYS:` overlay, and bootstrap HostFS mediation belong to `dos.library` and the trusted-internal hostfs bridge, not to normal application syscalls.
...
- Flat `IE64PROG` application launch from user mode. The remaining user-visible executable contract is the validated runtime ELF path.

**M15 runtime snapshot:**
...
- the M15 boot/demo path reached a richer text-mode system:
...
**M15.3 layered assigns + writable `SYS:` overlay runtime snapshot:**
...
- module-class framework is the runtime-facing data model for libraries, handlers, devices, and resources. Base M16 documented the staging state as module-class framework (`library` first; `device`/`handler`/`resource` reserved); M16.2 extends the same registry/lifecycle model to `handler`, `device`, and `resource` rows.
...
- `MODF_RESIDENT`, `MODF_COMPAT_PORT`, `MODF_ASLR_CAPABLE`, and `SIGF_MODDEAD` are shipped public constants. M16.3 made `MODF_ASLR_CAPABLE` mandatory for DOS-loaded ELFs while still keeping strict `ET_EXEC`; M16.4 introduced the current relocation and ASLR contract.
...
- At the M16.4.2 boundary, the protected-module model was preserved without adding command-resident executable caching, startup-sequence lifecycle hacks, raw cross-task code sharing, or a new public syscall slot. M16.4.3 later adds DOS-owned command residency as described in the dedicated section near the end of this document.

**M16.1 IOSM / VERSION summary:**

- `IOS_VERSION_*` in `sdk/include/iexec.inc` is the single source of truth for the OS-wide version, currently `1.16.7`.
...
- The default `VERSION` command output is canonicalised to `IntuitionOS 1.16.7`, `exec.library 1.16.7 (DATE)`, and the IOSM copyright string.
...
- M16.2 kept the then-current `ET_EXEC` placement contract. PIE-capable shipped ELF consistency was reserved for M16.3; relocation and ASLR/randomised placement were reserved for M16.4. The current M16.4.3 runtime contract is the `ET_DYN` / `PT_NOTE` contract described above.
...
- Discover effective IE64-visible RAM during kernel initialisation through the new IE64 `CR_RAM_SIZE_BYTES` control register and boot/system information block.
...
- **Code pages**: `P|X|U` or `P|R|X|U` - executable, never writable; M15.6 R4 makes execute-only user text a first-class contract
...
At boot, the kernel loads bundled program images into free task records via the internal `load_program` helper. From Milestone 5 onward, additional tasks can also be created at runtime via `CreateTask` (syscall #5). **M13 phase 2 changes the image-placement half of that contract**: the kernel still claims a free internal task slot / TCB row and assigns a monotonic public task ID, but it now allocates code, stack, data, and PT backing dynamically inside the reserved user image/PT windows instead of deriving addresses from `task_id * USER_SLOT_STRIDE`. Tasks exit via `ExitTask` (syscall #34), which cleans up ports, signals, and the task's dynamically allocated image/PT pages before freeing the task slot.

### 3.2 Task Control Block (TCB)

Each task is described by a 32-byte kernel-private TCB row. GPRs are not stored inline in the TCB; timer preemption saves the user GPR frame on the task's user stack and marks `gpr_saved`.

| Offset | Size | Field | Description |
|--------|------|-------|-------------|
| `+$000` | 8 | `pc` | Saved program counter |
| `+$008` | 8 | `usp` | Saved user stack pointer |
| `+$010` | 4 | `sig_alloc` | Allocated signal bits |
| `+$014` | 4 | `sig_wait` | Signal mask being waited for, or 0 |
| `+$018` | 4 | `sig_recv` | Pending received signals |
| `+$01C` | 1 | `state` | Task state byte |
| `+$01D` | 1 | `waitport` | WaitPort port ID, or `WAITPORT_NONE` (`0xFF`) |
| `+$01E` | 1 | `gpr_saved` | 1 when a timer preemption frame is saved on the user stack |
| `+$01F` | 1 | `tearflags` | Internal teardown flags |

PTBR per task is stored in a separate array at `KD_PTBR_BASE`.
...
| 6 | `ERR_AGAIN` | Resource temporarily unavailable, for example no message on a port |
| 7 | `ERR_TOOLARGE` | Allocation or request too large |
| 8 | `ERR_EXISTS` | Named object already exists, for example duplicate public port name |
| 9 | `ERR_FULL` | Global resource pool exhausted |
| 10 | `ERR_QUOTA` | Caller reached its per-task quota limit |
| 11 | `ERR_LIB_VERSION` | Library exists but is below the requested minimum version |
| 12 | `ERR_UNSUPPORTED` | Operation is known but rejected by class or policy |
...
| 13 | `Signal` | R1=public_task_id, R2=signal_mask -> R2=err | **Implemented** (`nucleus`) |
...
| # | Name | Signature | Status |
|---|------|-----------|--------|
| 15 | `CreatePort` | R1=name_ptr (0=anon), R2=flags -> R1=port_id, R2=err | **Implemented (M7)** (`nucleus`) |
| 16 | `FindPort` | R1=name_ptr -> R1=port_id, R2=err | **Implemented (M7)** (`nucleus`) - the canonical named-port lookup call; slot 36 preserves the older `OpenLibrary` binary-compat entry point by redirecting here |
| 17 | `PutMsg` | R1=port_id, R2=type, R3=data0, R4=data1, R5=reply_port, R6=share_handle -> R2=err | **Implemented (M7)** (`nucleus`) |
| 18 | `GetMsg` | R1=port_id -> R1=type, R2=data0, R3=err, R4=data1, R5=reply_port, R6=share_handle, R7=sender_public_task_id | **Implemented (M7, R7 sender added in M12.5)** (`nucleus`) |
| 19 | `WaitPort` | R1=port_id -> same as GetMsg, blocks if empty | **Implemented (M7, R7 sender added in M12.5)** (`nucleus`) |
...
**Port naming (M7, widened in M12):** Ports can be created with a name up to `PORT_NAME_LEN` bytes including the terminator (`32` bytes in the current ABI) and the `PF_PUBLIC` flag. Public named ports are discoverable via `FindPort` (case-insensitive matching). Anonymous ports (name_ptr=0) are private and not findable. Duplicate public names return `ERR_EXISTS`. Ports are cleaned up on task exit (name and flags cleared, removed from FindPort). Port IDs are byte-sized internally; `0xFF` is reserved as `WAITPORT_NONE`, so the public ceiling is 255 IDs, not the old M7 eight-port table.

**Message format (M7):** Messages are 32 bytes, kernel-copied, with type (4B), sender public task ID (4B), data0 (8B), data1 (8B), reply_port (2B, 0xFFFF=none), and share_handle (4B, 0=none). `ReplyMsg` is a convenience wrapper that sends to the reply_port with share_handle support.
...
| 28 | `MapIO` | R1=base_ppn, R2=page_count -> R1=mapped_va, R2=err | **Implemented (M9, trust-gated in M12.5)** (`bootstrap, trusted-internal`). Requires a covering hardware grant; normal applications do not receive ambient MMIO access. Mapped I/O pages are installed user-readable/user-writable and non-executable; unlike `MapShared`, this syscall does not take `MAPF_*` flags in the current source. |
| 29 | -- | -- | Removed M11.5 (was `MapVRAM`; unallocated hole) |
...
| # | Name | Signature | Status |
|---|------|-----------|--------|
| 35 | `ExecProgram` | R1=launch_desc_ptr, R2=launch_desc_size, R3=args_ptr, R4=args_len -> R1=public_task_id, R2=err | **Implemented (descriptor-only as of M14.2 phase 1)** (`nucleus`) |
| 36 | `OpenLibrary` | R1=name_ptr -> R1=port_id, R2=err | **Binary-compat redirect to `SYS_FIND_PORT` (M11.5)**. `SYS_OPEN_LIBRARY` remains constant 36 in `sdk/include/iexec.inc`; kernel slot 36 dispatches to `.do_find_port` via a one-instruction redirect so older IE64 binaries still link. New code uses `SYS_FIND_PORT` directly. |
...
**ExecProgram (35)** - current M14.2 ABI: Launches a new task from a caller-mapped M14 launch descriptor. R1=launch_desc_ptr (user VA, must be >= `0x600000`), R2=launch_desc_size, R3=args_ptr, R4=args_len. The handler runs under the **caller's** page table while validating the descriptor header and any supplied args range with the M15.4 permission-aware read helpers (`validate_user_read_range` rather than the old generic `P|U` check), then launches through the descriptor path that preserves ELF target VAs, protections, and entry point. Returns R1=new public task ID, R2=err. Returns `ERR_BADARG` for unmapped/kernel-only ranges, malformed descriptors, flat-image callers, args_len > 256, or pointer arithmetic overflow; returns `ERR_NOMEM` when the dynamic image/PT windows or page allocator are exhausted.
...
**M15.4/M15.6 user-pointer validation helpers**: `validate_user_read_range`, `validate_user_write_range`, and `validate_user_exec_range` walk the caller's page table once per page in the requested byte range and require the matching permission mask (`P|R|U`, `P|R|W|U`, or `P|X|U`). M15.6 R4 intentionally drops the old implicit read requirement for executable ranges so execute-only user text validates cleanly. Unmapped pages, kernel-only pages, permission mismatches, and pointer-arithmetic overflows all fail with `ERR_BADARG`.
...
**OpenLibrary (36) - M11.5 binary-compat redirect**: Originally a distinct M9 syscall for AmigaOS-style library discovery. M11.5 collapsed its implementation into `SYS_FIND_PORT`. `SYS_OPEN_LIBRARY` remains constant 36 in `sdk/include/iexec.inc`, and the kernel dispatcher slot 36 is retained as a one-instruction redirect (`bra .do_find_port`) so any IE64 binary that hardcoded the number 36 still works. Calling slot 36 produces an identical result to calling slot 16. New code uses `SYS_FIND_PORT` directly. Guarded by `TestIExec_OpenLibrary_DispatcherCollapse`. See § 5.11 "Exec Boundary".
...
|---------|------|---------|--------|
| 0 | SYSINFO_TOTAL_PAGES | Allocator-pool page count (`ALLOC_POOL_PAGES`, currently 3584) | **Implemented** |
| 1 | SYSINFO_FREE_PAGES | Free allocator-pool pages available | **Implemented** |
| 2 | SYSINFO_TICK_COUNT | Kernel tick count (incremented on each timer interrupt) | **Implemented** |
| 3 | SYSINFO_CURRENT_TASK | Current public task ID | **Implemented** |
| 4 | SYSINFO_QUOTA_CURRENT | Current quota usage for task/kind query | **Implemented** |
| 5 | SYSINFO_QUOTA_LIMIT | Current quota limit for kind query | **Implemented** |
| 6 | SYSINFO_PORT_NAME_BY_INDEX | Enumerate public port names into a 32-byte caller buffer | **Implemented** |
| 7 | SYSINFO_ASLR_IMAGE_BASE | Trusted-internal ASLR image-base selection for dos.library | **Implemented** |
| 8 | SYSINFO_GUEST_RAM_PAGES | Guest RAM size in 4 KiB pages from SYSINFO MMIO | **Implemented** |
...
**Signal (13)**: Sends signals to another task. R1=target public task ID, R2=signal mask. The kernel resolves the public ID to a live internal slot, then OR's the mask into the target task's `sig_recv` (pending signals). If the target is in WAITING state and any newly-set bit matches its `sig_wait` mask, the target is moved to READY and will receive the matched signals as the return value of its pending `Wait` call. Returns R2=ERR_OK on success, R2=ERR_BADARG if the target public task ID is invalid.
...
1. It requires the kernel's privileged state - page tables, scheduler queues, trap frames, IRQ routing, MMU operations, or MMIO grant enforcement, **AND**
...
| 16 | `FindPort` | `nucleus` | Public name registry - canonical lookup path used by new source |
...
| 36 | `OpenLibrary` | `nucleus` (binary-compat redirect to slot 16) | `SYS_OPEN_LIBRARY` remains constant 36; slot 36 dispatches to `.do_find_port` via a one-instruction redirect so any IE64 binary hardcoded to call number 36 still works. New code uses `SYS_FIND_PORT` directly. The Amiga-shaped programming model (`OpenLibrary("dos.library")`) is preserved without keeping a second implementation. |
...
**5.11.6 OpenLibrary rationale.** Classic AmigaOS implemented `OpenLibrary` as an `exec.library` jump-table call, not a trap. In IntuitionOS, new source uses `SYS_FIND_PORT` for named service lookup. `SYS_OPEN_LIBRARY` remains the slot-36 compatibility constant, and slot 36 in the dispatcher is retained as a one-instruction redirect to `.do_find_port` so any IE64 binary compiled before M11.5 continues to work. A regression test (`TestIExec_OpenLibrary_DispatcherCollapse`) guards the contract that calling slot 36 produces an identical result to calling slot 16.
...
**5.12.1 Trust model.** `KD_HWRES_TASK` is a u32 at offset 49336 in the kernel data page and holds the broker public task ID, initialised to `0xFFFFFFFF` (sentinel = unclaimed) at boot. The first task to call `SYS_HWRES_OP` with verb `HWRES_BECOME` claims broker identity; subsequent `HWRES_BECOME` calls return `ERR_EXISTS`. `HWRES_CREATE` is gated by `current_public_task_id == KD_HWRES_TASK` - only the broker may write a grant. There are no groups, ACLs, users, or capability masks beyond this single distinction. M12.6 may add more privileged identities; M12.5 deliberately stops at one because no second consumer exists yet.
...
| `HWRES_CREATE` | 1 | Write a grant row for `(target_public_task_id, region_tag, ppn_lo, ppn_hi)` (R1..R4). Gated by `current_public_task_id == KD_HWRES_TASK`. |
| `HWRES_REVOKE` | 2 | Reserved for M13. Returns `ERR_BADARG` in M12.5. |
| `HWRES_TASK_ALIVE` | 3 | Query whether a public task ID still resolves to a live task. R1 = target public task ID; returns R1 = 1 if the ID resolves, else 0. Gated by `current_public_task_id == KD_HWRES_TASK`. Used by the broker to lazily reclaim stale per-tag owner entries when a previous grantee has exited (the kernel grant chain and `KD_HWRES_TASK` are auto-cleaned by `kill_task_cleanup`, but the broker's private owner-list state is not - this verb gives the broker enough kernel state to reconcile). |
...
- M14.1 phase 2 runtime snapshot:
...
4. `WaitPort` loop. Each `HWRES_MSG_REQUEST` (data0 = 4-byte region tag, data1 = unused/ignored) goes through:
   - **Sender identity** - the kernel-trusted sender public task ID arrives in `R7` from `SYS_WAIT_PORT`/`SYS_GET_MSG`. The broker uses R7 (which is `KD_MSG_SRC` populated by `PutMsg`) and *ignores* anything the client may have stored in `data1`. M11.5+ message-port semantics make `MSG_SRC` unforgeable.
   ...
   - On grant, the broker calls `SYS_HWRES_OP`/`HWRES_CREATE` with the trusted sender public task ID and the resolved PPN range, then replies with `HWRES_MSG_GRANTED` whose data0 carries `(ppn_base<<32) | page_count`.
...
| Symbol | Source | Value | Bucket | Notes |
|---|---|---|---|---|
| `MMU_PAGE_SIZE` | `sdk/include/iexec.inc` | 4096 | A | MMU page size - part of the page-table format. |
| `MMU_PAGE_SHIFT` | `sdk/include/iexec.inc` | 12 | A | log2 of `MMU_PAGE_SIZE`. |
| `MMU_NUM_PAGES` | `sdk/include/iexec.inc` | 8192 | A | Current M16-era total physical RAM pages on the 32 MB IExec target. The planned MAX RAM ABI migration replaces this fixed value with a value derived from effective IE64 RAM. |
| `KERN_PAGES` | `sdk/include/iexec.inc` | 384 | A | Kernel reserved page range. Bounded by the layout of `KERN_PAGE_TABLE` / `KERN_DATA_BASE` / kernel binary / kernel stack. |
| `KERN_PAGE_TABLE` | `sdk/include/iexec.inc` | 0x07D000 | A | Fixed kernel page-table location, shifted down in M15.6 R1 so the kernel stack can have a dedicated guard page. |
| `KERN_DATA_BASE` | `sdk/include/iexec.inc` | 0x08D000 | A | Fixed kernel data-page base. |
| `KERN_STACK_TOP` | `sdk/include/iexec.inc` | 0x0A0000 | A | Top of the kernel stack inside the kernel reserved range. |
| `MAX_TASKS` | `sdk/include/iexec.inc` | 255 | A | **M13 phase 4:** the old layout-bound 32-task cap is gone. The remaining fixed task-state tables now run to the current 8-bit internal-slot ABI ceiling (`255`, with `0xFF` reserved by a few byte-sized kernel fields such as `KD_PORT_OWNER` sentinels). Task image/PT backing now spills into allocator-pool pages once the legacy fixed windows are exhausted, so the practical limit is no longer the old 32-slot VA layout. |
| `KD_TASK_STRIDE` | `sdk/include/iexec.inc` | 32 | A | TCB layout - bounded by the on-data-page field offsets, not arbitrary. |
| `KD_PTBR_BASE` | `sdk/include/iexec.inc` | 8224 | A | PTBR array offset after the 255-row TCB array. Layout-derived from `KD_TASK_BASE + MAX_TASKS * KD_TASK_STRIDE`. |
| `KD_PORT_STRIDE` | `sdk/include/iexec.inc` | 168 | A | Port size = 40-byte header + 4×32-byte messages. Bounded by `KD_PORT_FIFO_SIZE` × `KD_MSG_SIZE` + header. |
| `KD_PORT_INLINE_MAX` | `sdk/include/iexec.inc` | 32 | A | Number of port rows kept inline for fast-path access (M12.6 Phase C). The actual cap on ports is the allocator pool, reached via the overflow chain. |
| `KD_PORT_MAX` | `sdk/include/iexec.inc` | 32 | C → A | **Cap removed in M12.6 Phase C; symbol retained as legacy alias for `KD_PORT_INLINE_MAX`.** The original 32-row hard cap is gone - rows beyond 32 live in an overflow chain reached through `KD_PORT_OFLOW_HDR`. The 1-byte port ID ABI ceiling is 255 (0xFF reserved as `WAITPORT_NONE` sentinel). |
| `KD_PORT_FIFO_SIZE` | `sdk/include/iexec.inc` | 4 | B | Per-port message queue depth, not a global system cap. Tied to message-port flow control semantics; replaceable later but not load-bearing. |
| `KD_PORT_OFLOW_HDR` | `sdk/include/iexec.inc` | 51400 | A | Single global chain header for the port overflow chain (M12.6 Phase C), shifted by later 255-task and hardening layout growth. |
| `PORT_NAME_LEN` | `sdk/include/iexec.inc` | 32 | A | ABI - every port-name field across the kernel + protocol uses this width. Bumping it later would be a flag-day change to message-port headers. |
| `KD_MSG_SIZE` | `sdk/include/iexec.inc` | 32 | A | ABI - message size is part of the cross-task IPC contract. |
| `REPLY_PORT_NONE` | `sdk/include/iexec.inc` | 0xFFFF | A | Sentinel value, not a cap. |
| `WAITPORT_NONE` | `sdk/include/iexec.inc` | 0xFF | A | Sentinel value, not a cap. |
| `USER_PT_BASE` | `sdk/include/iexec.inc` | 0x800000 | A | Base of the fixed user page-table window. The fixed window is 2 MiB (`USER_PT_BASE..USER_DYN_BASE`); tasks beyond that window spill PT backing into allocator-pool pages. |
| `USER_SLOT_STRIDE` | `sdk/include/iexec.inc` | 0x10000 | A | Per-task slot stride - bounded by the layout of code/stack/data pages inside one slot. |
| `USER_DYN_BASE` / `USER_DYN_END` | `sdk/include/iexec.inc` | 0xA00000 / 0x1200000 | A | Dynamic VA range - **M12.6 Phase E security fix**: split from the allocator pool into a disjoint VPN range. Previously the user-dyn window aliased the entire allocator pool (`0xA00000..0x2000000`), which let unprivileged tasks overwrite the supervisor-only pool PTEs that `build_user_pt` copies into every user PT and pivot the kernel chain walkers (running on the user PT) into attacker-controlled memory. The fix gives user-dyn the bottom half (`0xA00000..0x1200000`, 8 MiB, VPN `0xA00..0x11FF`) and the allocator pool the top half (PPN `0x1200..0x1FFF`, 14 MiB). The two ranges are now disjoint at the layout level - see `TestIExec_PortChain_DisjointFromUserDyn`. |
| `USER_DYN_PAGES` | `sdk/include/iexec.inc` | 768 | B | Per-allocation cap for `AllocMem`, not per-task. Documents the largest single chunk a task can ask for. Replaceable. |
| `ALLOC_POOL_BASE` | `sdk/include/iexec.inc` | 0x1200 | A | First allocable physical page - **M12.6 Phase E security fix**: was 0xA00 (split user-dyn and pool into disjoint VPN ranges so user `SYS_ALLOC_MEM` calls cannot ever overwrite the supervisor-only pool PTEs in user PTs). Pool now starts at PPN 0x1200, immediately above the user-dyn VA window which ends at `USER_DYN_END = 0x1200000`. M12.6 Phase D had earlier bumped this from 0x800 to 0xA00 to make room for the doubled `USER_PT_BASE` region. |
| `ALLOC_POOL_PAGES` | `sdk/include/iexec.inc` | 3584 | A | **M12.6 Phase E security fix**: was 5632 (lost 2048 pages = 8 MiB to the user-dyn VA window so user-dyn and the allocator pool are now disjoint at the layout level). Pool spans pages `0x1200..0x1FFF` = 14 MiB. Bounded by `MMU_NUM_PAGES - ALLOC_POOL_BASE` in the current 32 MB ABI. MAX RAM must derive this from effective RAM minus reserved ranges. M12.6 Phase D had earlier shrunk this from 6144 to 5632 for the user PT region. |
| `KD_PAGE_BITMAP` / `KD_PAGE_BITMAP_SZ` | `sdk/include/iexec.inc` | 15640 / 800 bytes | A | Bitmap size derived from `ALLOC_POOL_PAGES` rounded to whole bytes. The offset is layout-derived and shifted by later kernel-data growth. |
| `KD_REGION_INLINE_MAX` | `sdk/include/iexec.inc` | 8 | A | Number of region rows kept inline per task for fast-path access (M12.5). The actual cap on regions per task is the allocator pool, reached via the overflow chain. |
| `KD_REGION_TASK_SZ` | `sdk/include/iexec.inc` | 128 | A | Inline byte stride per task (8 × 16). With M12.5's overflow chain this is now the inline range size, not a cap on regions per task. |
| `KD_REGION_MAX` | `sdk/include/iexec.inc` | 8 | C → A | **Cap removed in M12.5; symbol retained as legacy alias for `KD_REGION_INLINE_MAX`.** Rows beyond 8 live in a per-task overflow chain reached through `KD_REGION_OVERFLOW_HEAD`. |
| `KD_REGION_OVERFLOW_HEAD` | `sdk/include/iexec.inc` | 49352 | A | Per-task overflow chain header array base (M12.5), shifted by later 255-task and hardening layout growth. |
| `KD_REGION_OFLOW_STRIDE` | `sdk/include/iexec.inc` | 8 | A | Per-task overflow header stride. Layout-derived. |
| `KD_REGION_STRIDE` | `sdk/include/iexec.inc` | 16 | A | Region row size, bounded by the row field layout. |
| `KD_SHMEM_INLINE_MAX` | `sdk/include/iexec.inc` | 16 | A | Number of shmem rows kept inline for fast-path access (M12.6 Phase B). |
| `KD_SHMEM_MAX` | `sdk/include/iexec.inc` | 16 | C → A | **Cap removed in M12.6 Phase B; symbol retained as legacy alias for `KD_SHMEM_INLINE_MAX`.** Rows beyond 16 live in an overflow chain reached through `KD_SHMEM_OFLOW_HDR`. The 1-byte shmem id ABI ceiling is 255. |
| `KD_SHMEM_STRIDE` | `sdk/include/iexec.inc` | 16 | A | Shared-object row size, bounded by row layout. |
| `KD_SHMEM_OFLOW_HDR` | `sdk/include/iexec.inc` | 51392 | A | Single global chain header for the shmem overflow chain (M12.6 Phase B), shifted by later kernel-data growth. |
| `KD_HWRES_TASK` | `sdk/include/iexec.inc` | u32 at offset 49336 | A | Broker public task ID slot in the kernel data page. `0xFFFFFFFF` means unclaimed. |
| `KD_GRANT_TABLE_HDR` | `sdk/include/iexec.inc` | 8 bytes at offset 49344 | A | Chain header for the grant table (M12.5). The grant chain itself has no fixed-row cap. |
| `SYS_HWRES_OP` | `sdk/include/iexec.inc` | slot 38 | A | Trusted-internal verb-multiplexed broker primitive (M12.5). Slot number is part of the syscall ABI. |
| `IEXEC_HEARTBEAT_INTERVAL` | `sdk/include/iexec.inc` | 64 | A | Tunable - debug-only kernel heartbeat tick rate, not a system cap. |
| `IMG_HEADER_SIZE` | `sdk/include/iexec.inc` | 32 | A | IE64PROG image format ABI. |
...
| `PROGTAB_ENTRY_SIZE` | `sdk/include/iexec.inc` | 24 | A | Program table row layout, bounded by row fields. |
| `PROGTAB_BOOT_COUNT` | `sdk/include/iexec.inc` | 3 | A | Number of programs auto-loaded at boot - this is a *configured policy*, not an arbitrary cap. The number is "the count of strict-boot services," currently 3 (console.handler, dos.library, Shell). |
| `TERM_IO_PAGE` | `sdk/include/iexec.inc` | 0xF0 | A | Hardware MMIO page address. |
| `DATA_ARGS_OFFSET` / `DATA_ARGS_MAX` | `sdk/include/iexec.inc` | 3072 / 256 | B | Per-program argument-passing layout inside the program data page. Tied to the M9 `SYS_EXEC_PROGRAM` ABI. |
...
| `DOS_NAME_LEN` | `sdk/include/iexec.inc` | 32 | A | Filesystem ABI - bounded by the in-table filename field. |
...
| `INTUI_WIN_TITLE_H` | `sdk/include/iexec.inc` | 16 | A | Window title bar height in pixels - bounded by the embedded Topaz 8×16 font glyph height. |
| `INTUI_WIN_BORDER` | `sdk/include/iexec.inc` | 2 | A | Window bevel thickness - visual constant, not a cap. |
| `SIG_SYSTEM_MASK` | `sdk/include/iexec.inc` | 0xFFFF | A | Bit-field mask, ABI - system signals are bits 0-15, user signals 16-31, bounded by the 32-bit signal-word width. |
...
| 1 | `SIGF_MODDEAD` | An opened protected module or library owner died |

`SIG_SYSTEM_MASK` reserves bits 0-15 for system use. Bits other than `SIGF_PORT` and `SIGF_MODDEAD` are reserved in the current include contract; timer, abort, and child-exit system signals are not exported today.
...
- `Signal(public_task_id, mask)` sets bits in the target task's pending signal word. If the target is in WAITING state and any newly-set bit matches its wait mask, the target is moved to READY.

---

## 7. Port/Message Model

Ports are kernel-managed FIFO message queues, modelled after Amiga MsgPorts.
...
- An owning task
- The shared system signal `SIGF_PORT` is raised on the owner when a message arrives
- A FIFO message queue

### 7.2 Message Passing

Messages are fixed-size (32 bytes) and kernel-copied. Each message contains: type (4 bytes), sender public task ID (4 bytes), data0 (8 bytes), data1 (8 bytes), reply_port (2 bytes, 0xFFFF = none), and share_handle (4 bytes, 0 = none). `PutMsg` writes the message directly into the target port's kernel-managed FIFO (4 slots per port). `GetMsg` dequeues the oldest message and returns all fields in R1-R7, with `R7` carrying the sender public task ID. This fixed-size model is simple and assembly-friendly.
...
No public user-space timer API is implemented in the current syscall surface. Slots 22 and 23 are reserved holes after the M11.5 cleanup. The IE64 hardware timer is used by the kernel for scheduler ticks and preemption.

### 8.1 Delta Queue

The delta-queue design below is a future API sketch, not current runtime behaviour. A future implementation would maintain a sorted delta queue of pending timer requests, with each entry storing the remaining ticks until expiry as a delta from the previous entry.

### 8.2 Timer API

- `AddTimer(ticks, signal_mask)`: future one-shot timer operation. When it expires, the specified signal bits would be set on the calling task.
- `RemTimer(handle)`: future cancellation operation.

The hardware timer (`CR_TIMER_PERIOD`, `CR_TIMER_COUNT`, `CR_TIMER_CTRL`) drives the scheduler tick. The kernel programs it during boot and uses each interrupt for preemption. It does not currently deliver public timer objects.

---

## 9. Handle Model (Future)

No generic public handle-table API is implemented in the current syscall surface. Slots 24 and 25 are reserved holes after the M11.5 cleanup; `CloseHandle` and `DupHandle` are not exported today. Current opaque values are protocol-specific, for example share handles, module/library tokens, port IDs, and DOS handles.

### 9.1 Handle Table

A future generic handle table would map 32-bit handles to kernel objects. Handles would be opaque integers, not pointers, and the kernel would validate handle ownership on every syscall that consumes one.

### 9.2 Handle Types

| Type | Value | Description |
|------|-------|-------------|
| `HANDLE_TASK` | 1 | Future task reference handle; no current `FindTask` syscall exists |
...
- `CloseHandle(handle)`: future release operation. If it were the last reference to the underlying object, the object would be destroyed according to its class rules.
- `DupHandle(handle)`: future duplication operation.

---

## 10. Bootstrap Sequence

The current kernel boots in supervisor mode at `$1000` (`PROG_START`) and performs the following high-level steps. Exact offsets come from `sdk/include/iexec.inc`; the summary below intentionally avoids stale milestone-era addresses.

```
 1. Set trap vector, interrupt vector, and kernel stack pointer

 2. Build the kernel page table at KERN_PAGE_TABLE:
    - identity-map pages 0-383 ($000000-$17FFFF) as supervisor-only
    - install permission-specific mappings for kernel text, kernel data,
      kernel stack, the kernel PT/data windows, low I/O, and required
      supervisor carve-outs

 3. Initialise kernel data:
    - current_task = 0
    - tick_count = 0
    - num_tasks = 0
    - all MAX_TASKS TCB slots = FREE
    - inline and overflow headers for ports, shared memory, regions,
      grants, quotas, hooks, and module registry state are reset

 4. Enable the MMU using the kernel page table, with the supervisor
    access guards expected by the hardening contract

 5. Keep the legacy printable kernel boot banner disabled:
    - the `boot_banner` string is empty in the current source
    - visible version text is produced by the `VERSION` command path, not by an early kernel banner

 6. Launch the strict boot services:
    - validate the embedded or host-backed runtime ELF metadata
    - find a free internal task slot / TCB row and assign a public task ID
    - allocate code/stack/data/PT backing for the task image
    - apply the accepted local relocation ABI for ET_DYN images where present
    - copy/load code and data into allocated user pages
    - build the task's page table
    - write the startup block and initialise the task's TCB and PTBR entry
    - increment num_tasks
    - strict boot services must load successfully or the kernel panics

 7. Bring up the trusted-internal DOS and module launch path. Later
    protected libraries, handlers, devices, resources, and commands use
    the current ET_DYN / PT_NOTE runtime ELF contract.
...
| `Exec->ThisTask` | `GetSysInfo(SYSINFO_CURRENT_TASK)` | No direct pointer to TCB from user space; the old `FindTask` and `GetTaskInfo` syscall slots are removed holes |
...
| `OpenLibrary()`/`CloseLibrary()` | Named service lookup through `FindPort` for new source; slot-36 compatibility redirect for older binaries | `OpenLibrary("dos.library")` is preserved as the Amiga-shaped programming model. New source uses `SYS_FIND_PORT`; `SYS_OPEN_LIBRARY` remains constant 36 and dispatches to `.do_find_port` for binary compatibility. |
| Device I/O (`DoIO`/`SendIO`) | Device/handler/resource IPC plus grant-gated `MapIO` for trusted services | Normal application code uses service protocols; raw MMIO mapping is not an ambient user facility |
| `tc_Node.ln_Pri` scheduling | Not implemented | Scheduler is round-robin over task slots; priority scheduling remains future work |
...
- Self-sufficient boot: kernel builds its own page tables, creates user tasks, and initialises all scheduler state - no host-side setup required
...
- Boot banner and early debug terminal output were introduced here. The current legacy `boot_banner` string is disabled/empty; visible canonical version text is produced by the `VERSION` command path.
...
- Physical page allocator: bitmap-based contiguous allocation. The original M6/M11 pool shape has since been replaced by the current M12.6 Phase E allocator pool: `ALLOC_POOL_PAGES = 3584`, spanning PPN `0x1200..0x1FFF`.
- Per-task dynamic VA windows: the old M11 per-task stride (`$800000 + task_id * $300000`) has since been replaced. The current user-dynamic VA window is global per address space (`USER_DYN_BASE..USER_DYN_END = 0xA00000..0x1200000`), with isolation provided by per-task page tables.
...
- `GetSysInfo` extended: `SYSINFO_TOTAL_PAGES` (0) now returns `ALLOC_POOL_PAGES`, `SYSINFO_FREE_PAGES` (1) returns the current free allocator-pool count, and `SYSINFO_CURRENT_TASK` (3) returns the calling task's public task ID.
...
- **Named public ports**: `CreatePort` extended with R1=name_ptr, R2=flags (PF_PUBLIC). Names were 16 bytes in the M7 milestone and are `PORT_NAME_LEN = 32` bytes in the current ABI. Public named ports are discoverable via `FindPort`. Anonymous ports (name_ptr=0) remain private. Duplicate public names return ERR_EXISTS. Validate-then-commit: slot not marked valid until all validation passes.
...
- **Boot-time loader** (`load_program`): validates image header (magic, sizes, alignment, truncation check), finds a free internal task slot / TCB row, assigns a public task ID, allocates dynamic code/stack/data/startup/PT placement inside the reserved image/PT windows, copies code and data into the allocated pages, builds the page table, initialises the TCB, writes the startup block into the dedicated startup page, and places that startup-page VA at `0(sp)` before first entry. Validate-then-commit pattern. Not a syscall - kernel-internal only.
...
- **New syscalls** (as shipped in M9; later evolved - see M10/M11/M11.5 sections):
  ...
  - `SYS_OPEN_LIBRARY` (36): added in M9 as a distinct AmigaOS-style `OpenLibrary` syscall. **M11.5 collapsed its implementation into `SYS_FIND_PORT`**: slot 36 in the kernel dispatcher is retained as a one-instruction binary-compat redirect to `.do_find_port` so any IE64 binary that hardcoded the number 36 still works. `SYS_OPEN_LIBRARY` remains constant 36 in `sdk/include/iexec.inc`; new code uses `SYS_FIND_PORT` directly. See § 5.11 "Exec Boundary" for the rationale.
...
- **Two name resolution subroutines**:
  - `resolve_command_name` (used by `DOS_RUN`): if no colon found, default to **C:** (prepend `C/`). Matches AmigaOS command search path behaviour.
  - `resolve_file_name` (used by `DOS_OPEN`/`READ`/`WRITE`/`DIR`): if no colon found, **bare name** (no prefix). Matches AmigaOS file access behaviour.
...
- Fills the surface with a solid colour, sends one `GFX_PRESENT`, then enters its main event loop.
...
The user can then type `GFXDEMO` to launch `C/GfxDemo`, which fills the framebuffer with a solid colour and waits for Escape.
...
- **`SYS_OPEN_LIBRARY` collapsed to `SYS_FIND_PORT`.** The dispatcher slot 36 is retained as a one-instruction redirect (`bra .do_find_port`) for binary compatibility with any IE64 binary that hardcoded the number 36. The "deletion" is conceptual: 36 is no longer a distinct implementation path, but the dispatcher slot and `SYS_OPEN_LIBRARY equ 36` constant stay.
...
- Existing yield-between-polls behaviour absorbs the no-line-ready case naturally. The `CON_MSG_READLINE` request/reply protocol is unchanged, so all existing readline-using code (the shell, the seven existing readline tests) keeps working without modification.
...
- Every `syscall #SYS_OPEN_LIBRARY` call site in the boot programs (12 sites in `dos.library`, `console.handler`, `input.device`, `graphics.library`, `shell`, `C/GfxDemo`) was rewritten as `syscall #SYS_FIND_PORT`, so new readers of the boot programs see the canonical lookup call. Slot 36 remains only for older binaries that already encoded it.
...
Every behaviour-changing phase landed test-first.
...
- At the M11.5 boundary there was no `hardware.resource` refactor of `SYS_MAP_IO`'s allowlist. That wart is documented in 5.11.4 and was resolved in M12.5 by the grant-table model.
...
5. `INTUITION_OPEN_WINDOW(w=320, h=200, x=320, y=170, idcmp_port=mine, share=surface_share)` → `window_handle`
   (Window is centred on the 960x540 screen.)
...
All decoration is painted via the `.intui_fillrect` helper subroutine - a small `jsr`/`rts` routine in the intuition.library code section that fills a rectangle with a single colour. It hardcodes the 960x540 screen stride (3840) and reads the screen surface VA from `data[200]`. Rect coordinates are passed in `r6..r9` and colour in `r17`.
...
- **`hardware.resource` and de-publicisation of `SYS_MAP_IO`.** Still a documented impurity at the M12 boundary; resolved in M12.5 by the grant-table model and user-space `hardware.resource` broker.
...
5. `SYS_EXEC_PROGRAM(temp_buf, image_size, args, args_len)` → kernel copies the image into a newly built task image, returns public_task_id.
...
7. Reply with the public_task_id.
...
`C:Resident` is the public command surface:

- no arguments lists both exec-owned resident inventory (`EXEC_MSG_LIST_RESIDENT_INVENTORY`) and DOS-owned resident command cache inventory (`DOS_RESIDENT_LIST`)
- `Resident <name> ADD` sets protected service residency through `SYS_SET_RESIDENT` when `<name>` contains a dot, and adds command residency through `DOS_RESIDENT_ADD` otherwise
- `Resident <name> REMOVE` clears the matching protected-service or command residency row
- `Resident ALL` is treated as inventory listing. `Resident ALL REMOVE` is deliberately rejected by the command. `Resident ALL ADD` is not a bulk operation in the current source; it follows the normal command-residency add path with the literal command name `ALL` and therefore succeeds only if such a command resolves and validates.
- successful mutation prints `Resident: added` or `Resident: removed`; absent rows print `Resident: not found`; unsupported class or policy cases print `Resident: unsupported target`

The DOS resident command cache is bounded by `DOS_RCMD_CACHE_MAX = 16` rows in `sdk/intuitionos/iexec/lib/dos_library.s`. A row stores a 32-byte IOSM public name, a 32-byte resolved command path, a DOS-owned image VA and byte size, a generation counter, and a copied 128-byte IOSM descriptor. Adding a command resolves it through the normal command resolver, reads the backing hostfs file, validates the M16.4 ASLR/runtime ELF contract, requires `IOSM_KIND_COMMAND`, and snapshots the bytes into DOS-owned memory. Removing a command frees the snapshot and clears the row. `DOS_RUN` probes the resident command cache before filesystem lookup; on hit, it launches the cached bytes through the same `SYS_BOOT_ELF_EXEC` / runtime-loader path used for non-cached commands. There is no raw cross-task code sharing and no bypass of ELF validation, ASLR, W^X, or startup-block construction.

`DOS_RESIDENT_LIST` writes an `RCLI` v1 page into the caller's one-page `MEMF_PUBLIC` buffer. Its 32-byte header carries magic `RCLI`, version, header size, record size, record count, bytes used, and flags. Each 64-byte record mirrors the resident inventory shape for command rows: IOSM name, IOSM kind, module class `0`, online/resident/non-registry-provider status bits, version fields, IOSM flags, and generation. Exec-owned protected modules continue to use `RSIV` records from `EXEC_MSG_LIST_RESIDENT_INVENTORY`; command rows are intentionally DOS-owned and are not inserted into the exec protected-module registry.

M16.4.3 does not add command-resident executable sharing, demand loading for protected non-library rows, dynamic linking, external-symbol relocation, raw cross-task pointers, a new public syscall slot, or a startup-sequence lifecycle shortcut. It preserves the M16 protected-module model and keeps command caching as an optimisation of DOS command lookup plus validated relaunch.
