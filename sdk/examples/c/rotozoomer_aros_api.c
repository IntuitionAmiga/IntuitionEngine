...
{
    BPTR fh = Open("PROGDIR:rotozoomtexture_api_c.raw", MODE_OLDFILE);
    LONG bytes_read;
    if (!fh) {
        return 0;
    }
...
int main(void)
{
    ...
    /* Open screen */
    {
        struct TagItem scr_tags[] = {
            ...
            { SA_Height, RENDER_H },
            { SA_Depth, 32 },
            { SA_Title, (IPTR)"Rotozoomer API (C)" },
            { SA_ShowTitle, FALSE },
            { SA_Quiet, TRUE },
            { TAG_DONE, 0 }
        };
    ...
    while (running) {
        ...
        /* WritePixelArray: back_buf -> screen RastPort */
        WritePixelArray(back_buf, 0, 0, LINE_BYTES,
                        &screen->RastPort, 0, 0, RENDER_W, RENDER_H,
                        RECTFMT_RGBA);
