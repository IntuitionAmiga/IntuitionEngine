...
SHOWREEL_IE64_ARTIFACTS := \
	...
	$(SHOWREEL_PREBUILT_DIR)/rotozoomer_nocpu.ie64 \
...
AB3D2_README := $(AB3D2_BUILD_DIR)/README.md
...
.PHONY: ie32asm ie64asm ie64dis ie64ld ie64-cproc ie64-ar ie64-ranlib ie32to64 m68kto64 test-m68kto64 rotozoom-textures nocpu-rotozoomer ula-boing-ie64 gem-rotozoomer emutos-rom aros-rom aros-ie-live-assets aros-ie-live-inputs aros-ie-toolchain-assets aros-release-assets aros-iewarp-library iewarp-service-worker iewarp-runtime-local-assets iewarp-runtime-assets arosvision-probe-tree arosvision-live-base arosvision-live-components arosvision-live-overlays arosvision-live-tree arosvision-probe-run emutos-probe emutos-release-rom iedoom iedoom-ie86 iedoom-ie68 basic basic-emutos cputest-musashi dist-ie64-toolchain-linux-amd64
...
# same IE32 JIT contract suite, including Chromium generated-module execution,
# direct-lowering provenance, and the interpreter boundaries for timer and
# observation-sensitive instructions.
IE32_JIT_TEST_REGEX := ^(TestIE32(JIT|StepOne|Retired|Wasm).*|TestScriptEngine_ShowreelDiagnosisScriptsParse)$$
test-ie32-jit-parity:
	@./scripts/require-go-test-inventory.sh "empty IE32 native JIT inventory" env GOEXPERIMENT=none $(GO) test -tags headless -list '$(IE32_JIT_TEST_REGEX)' .
	GOEXPERIMENT=none $(GO) test -tags headless -count=1 -run '$(IE32_JIT_TEST_REGEX)' .
...
# x86 benchmark wrappers over the generic bench-* flow. These keep the
# accepted workload selection stable across amd64 and native Linux/ARM64
# hosts while leaving placement policy and BENCH_ITEM caller-owned.
x86-bench-baseline:
	@$(MAKE) bench-baseline \
...
.PHONY: x64-live
x64-live: deb-intuitionengine-amd64-v3 x64-live-demos wasm dist-host-sdk-linux-amd64 dist-host-sdk-linux-arm64 dist-host-sdk-windows-amd64
...
x64-live-rebuild-golden: deb-intuitionengine-amd64-v3 x64-live-demos dist-host-sdk-linux-amd64 dist-host-sdk-linux-arm64 dist-host-sdk-windows-amd64
	@IE_APP_VERSION="$(APP_VERSION)" IE_PACKAGE_FILE="$(IE_DEB_DIR)/intuitionengine-amd64-v3_$(APP_VERSION)-1_amd64.deb" IE_REPOSITORY_KEYRING="$(IE_REPOSITORY_KEYRING)" X64_LIVE_OUT_DIR="$(X64_LIVE_DIR)" AROS_RELEASE_DIR="$(AROS_LIVE_DIR)" CHOCOLATE_DOOM_DIR="$(CHOCOLATE_DOOM_DIR)" IEDOOM_IE86="$(IEDOOM_IE86)" IEDOOM_IE68="$(IEDOOM_IE68)" IEDOOM_WAD="$(IEDOOM_WAD)" ./build_x64_ie_img.sh --rebuild-golden
...
x64-live-payload-check: x86-64-v3 sdk-build gem-rotozoomer arosvision-live-tree x64-live-aros-demos x64-live-ab3d2-assets x64-live-refman-pdfs x64-live-sdk-companion-pdfs intuitionos iedoom dist-host-sdk-linux-amd64 dist-host-sdk-linux-arm64 dist-host-sdk-windows-amd64
...
# The x64 payload check owns the one architecture-neutral IESHARE staging tree.
# Raspberry Pi images consume that exact tree instead of generating a copy.
...
rpi-live-payload-check: rpi-4-arm64 rpi-5-arm64 rpi-host-helper-arm64 x64-live-payload-check
	@scripts/build_rpi_live_image.sh --board pi4 --binary build/rpi4-live/IntuitionEngine-rpi4 --payload $(X64_LIVE_DIR)/work/ieshare-payload --check-payload
	@scripts/build_rpi_live_image.sh --board pi5 --binary build/rpi5-live/IntuitionEngine-rpi5 --payload $(X64_LIVE_DIR)/work/ieshare-payload --check-payload

rpi4-live-payload-check: rpi-live-payload-check
...
rpi4-live-qemu:
	@scripts/rpi4_virt_qemu.sh build/rpi4-live/intuition-engine-rpi4.img

rpi4-live-hardware-qemu:
...
iedoom-ie86:
	@if [ ! -f "$(CHOCOLATE_DOOM_DIR)/src/iedoom_build.sh" ]; then \
		echo "Error: missing IEDoom x86 builder: $(CHOCOLATE_DOOM_DIR)/src/iedoom_build.sh"; \
		exit 1; \
	fi
	@echo "Building IEDoom x86 guest image..."
	@cd "$(CHOCOLATE_DOOM_DIR)" && sh src/iedoom_build.sh "$(IEDOOM_IE86)"
	@test -f "$(CHOCOLATE_DOOM_DIR)/$(IEDOOM_IE86)" || { echo "Error: IEDoom x86 image was not produced: $(CHOCOLATE_DOOM_DIR)/$(IEDOOM_IE86)"; exit 1; }

iedoom-ie68:
	@if [ ! -f "$(CHOCOLATE_DOOM_DIR)/src/iedoom_build_m68k.sh" ]; then \
		echo "Error: missing IEDoom M68K builder: $(CHOCOLATE_DOOM_DIR)/src/iedoom_build_m68k.sh"; \
		exit 1; \
	fi
	@echo "Building IEDoom M68K guest image..."
...
web-demos:
	...
	@# Recursive manifest of the whole document-root assets folder (relative
	@# paths), which is the BASIC disk volume the browser preloads over HTTP.
	@(cd intuitionengine.com/assets && find . -type f ! -name MANIFEST ! -name '.gitkeep' ! -iname 'README.TXT' ! -iname 'mariokart*' ! -name 'intuition-engine-host-sdk-linux-amd64.tar.xz*' ! -name 'intuition-engine-host-sdk-linux-arm64.tar.xz*' ! -name 'intuition-engine-host-sdk-windows-amd64.zip*' | sed 's|^\./||' | LC_ALL=C sort) > intuitionengine.com/assets/MANIFEST
...
# Layer C: run the js/wasm test suite under Node. The handful of test files
# that reference native-only JIT symbols carry !js build tags, so the whole
...
test-cross-amd64-binaries:
	@echo "Building Linux and Windows amd64 VM binaries..."
	@$(MKDIR) -p $(CROSS_BUILD_DIR)
	@echo "== linux/amd64 full CGO =="
	@$(call build-linux-vm-binary,amd64,$(CC),$(CXX),,$(CROSS_BUILD_DIR)/$(CROSS_BINARY_PREFIX)-linux-amd64)
	@echo "== windows/amd64 pure Go novulkan =="
	@$(call build-purego-novulkan-vm-binary,windows,amd64,$(CROSS_BUILD_DIR)/$(CROSS_BINARY_PREFIX)-windows-amd64.exe)
...
ab3d2:
	...
	$(MKDIR) -p $(AB3D2_BUILD_DIR); \
	rm -f "$(AB3D2_BUILD_DIR)/$(AB3D2_ORIGINAL_BINARY_PREFIX)-"* "$(AB3D2_BUILD_DIR)/$(AB3D2_BINARY_PREFIX)-"*; \
	$(MAKE) prepare-ab3d2-embed AB3D2_SOURCE=$(AB3D2_ORIGINAL_SOURCE); \
	$(MAKE) test-cross-amd64-binaries CROSS_BUILD_DIR=$(AB3D2_BUILD_DIR) CROSS_BINARY_PREFIX=$(AB3D2_ORIGINAL_BINARY_PREFIX) VM_EMBED_TAGS="embed_ab3d2" EMBEDDED_AB3D2_START_FULLSCREEN=$(AB3D2_START_FULLSCREEN); \
	$(MAKE) prepare-ab3d2-embed AB3D2_SOURCE=$(AB3D2_SOURCE); \
	$(MAKE) test-cross-amd64-binaries CROSS_BUILD_DIR=$(AB3D2_BUILD_DIR) CROSS_BINARY_PREFIX=$(AB3D2_BINARY_PREFIX) VM_EMBED_TAGS="embed_ab3d2" EMBEDDED_AB3D2_START_FULLSCREEN=$(AB3D2_START_FULLSCREEN); \
	cp "$(AB3D2_README_SOURCE)" "$(AB3D2_README)"; \
	rm -f "$(AB3D2_ARCHIVE)"; \
	python3 scripts/package-ab3d2.py "$(AB3D2_BUILD_DIR)" "$(AB3D2_ARCHIVE)" \
		"$(AB3D2_ORIGINAL_BINARY_PREFIX)-linux-amd64" \
		"$(AB3D2_ORIGINAL_BINARY_PREFIX)-windows-amd64.exe" \
		"$(AB3D2_BINARY_PREFIX)-linux-amd64" \
		"$(AB3D2_BINARY_PREFIX)-windows-amd64.exe" \
		"README.md"
...
.PHONY: dist-host-sdk-linux-amd64 dist-host-sdk-linux-arm64 dist-host-sdk-windows-amd64 test-host-sdk test-host-sdk-arm64 test-host-sdk-windows-amd64 test-host-sdk-external
...
dist-host-sdk-windows-amd64:
	@bash ./scripts/dist-host-sdk-windows-amd64.sh
...
test-host-sdk-windows-amd64: dist-host-sdk-windows-amd64
	@tmp="$$(mktemp -d)"; trap 'rm -rf "$$tmp"' EXIT; unzip -q dist/intuition-engine-host-sdk-windows-amd64.zip -d "$$tmp"; bash ./scripts/test-installed-host-sdk.sh "$$tmp/intuition-engine-host-sdk-windows-amd64"
...
showreel-ie64: ie64asm rotozoom-textures nocpu-rotozoomer ula-boing-ie64
...
nocpu-rotozoomer: ie64asm ./sdk/examples/asm/rotozoomer_nocpu.asm ./sdk/examples/assets/rotozoomtexture_nocpu.raw ./sdk/examples/assets/music/yourlove.mid
	@echo "Building no-CPU IE64 bootstrap..."
	@$(MKDIR) -p $(SHOWREEL_PREBUILT_DIR)
	@$(SDK_BIN_DIR)/ie64asm -I sdk/include sdk/examples/asm/rotozoomer_nocpu.asm
	@mv sdk/examples/asm/rotozoomer_nocpu.ie64 $(SHOWREEL_PREBUILT_DIR)/
	@echo "Output: $(SHOWREEL_PREBUILT_DIR)/rotozoomer_nocpu.ie64"
