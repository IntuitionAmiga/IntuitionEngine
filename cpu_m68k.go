...
type M68KCPU struct {
	...
	*/
	...
	Running   bool
	Debug     bool
	_padding1 [20]byte // Cache line alignment reduces false sharing
	...
	prefetchSize     uint8
	currentIR        uint16
	pendingInterrupt uint8    // Set by external hardware; checked each instruction
	inException      bool     // Prevents double-fault situations
	...
	// Cache Lines 3+ - Synchronisation and Control
	mutex          sync.RWMutex // Read/write locks optimise concurrent access patterns
	interruptMutex sync.Mutex   // Prevents race conditions on interrupt state
	bus            MemoryBus
...
func (cpu *M68KCPU) ExecuteInstruction() {
	...
	instructionLimit := uint32(100000)
	...
	for cpu.Running {
		...
		if cpu.PC >= M68K_MEMORY_SIZE-M68K_LONG_SIZE {
			fmt.Printf("Error: Programme counter out of bounds: PC=0x%08X\n", cpu.PC)
			cpu.Running = false
			break
		}
		...
		// Process interrupts
		cpu.interruptMutex.Lock()
		ipl := uint8((cpu.SR & M68K_SR_IPL) >> M68K_SR_SHIFT)
		if cpu.pendingInterrupt > ipl && !cpu.inException {
			cpu.ProcessInterrupt(cpu.pendingInterrupt)
			cpu.pendingInterrupt = 0
		}
		cpu.interruptMutex.Unlock()
...
func (cpu *M68KCPU) Read8(addr uint32) uint8 {
	...
	if addr >= M68K_MEMORY_SIZE {
		fmt.Printf("Warning: Read from out-of-bounds address 0x%08X\n", addr)
		return 0
	}
...
func (cpu *M68KCPU) Read16(addr uint32) uint16 {
	...
	if addr >= M68K_MEMORY_SIZE-M68K_BYTE_SIZE {
...
func (cpu *M68KCPU) Write8(addr uint32, value uint8) {
	...
	if addr >= M68K_MEMORY_SIZE {
		fmt.Printf("Warning: Write to out-of-bounds address 0x%08X\n", addr)
		return
	}
...
func (cpu *M68KCPU) GetEffectiveAddress(mode, reg uint16) uint32 {
	...
	case M68K_AM_AR_INDEX: // Address Register Indirect with Index
		...
		if cpu.Debug {
			fmt.Printf("A%d with index: address=0x%08X\n", reg, addr)
		}
		return addr
...
func (cpu *M68KCPU) GetIndexWithExtWords(extWord uint16, baseAddr uint32, isAreg bool) uint32 {
	...
	default:
		if cpu.Debug {
			fmt.Printf("M68K: Invalid base displacement size %d at PC=%08x\n", bd, cpu.PC-M68K_WORD_SIZE)
		}
		cpu.ProcessException(M68K_VEC_ILLEGAL_INSTR)
		return 0
	}
	...
	if indLevel != 0 {
		...
		case 1: // [bd,An,Xn] - Memory indirect preindexed
			...
			if (extWord & 0x04) != 0 { // Check od bit
				...
				default:
					if cpu.Debug {
						fmt.Printf("M68K: Invalid outer displacement size %d at PC=%08x\n", extWord&0x03, cpu.PC-M68K_WORD_SIZE)
					}
					cpu.ProcessException(M68K_VEC_ILLEGAL_INSTR)
					return 0
				}
...
func (cpu *M68KCPU) ProcessException(vector uint8) {
	...
	if cpu.inException && (vector == M68K_VEC_BUS_ERROR || vector == M68K_VEC_ADDRESS_ERROR) {
	...
	if cpu.inException && vector != M68K_VEC_RESET {
		fmt.Printf("M68K: Exception %d during exception handling, deferring\n", vector)
		cpu.pendingInterrupt = vector
		return
	}
	...
	vecAddr := uint32(vector) * M68K_LONG_SIZE
	newPC := cpu.Read32(vecAddr)

	// Uninitialised vectors halt because no handler exists
	if newPC == 0 {
		if cpu.Debug {
			fmt.Printf("M68K: Uninitialised vector %d, using default handler\n", vector)
		}
		cpu.Running = false
		cpu.inException = false
		return
...
func (cpu *M68KCPU) ProcessInterrupt(level uint8) {
	...
	if level == M68K_VEC_LEVEL7-M68K_VEC_LEVEL1+1 || level > currentIPL {
		...
		if cpu.Debug {
			fmt.Printf("M68K: Interrupt level %d at PC=%08x\n", level, cpu.PC)
		}
	}
}
...
func (cpu *M68KCPU) ExecMove(srcMode, srcReg, destMode, destReg uint16, size uint16) {
	...
	if srcMode == M68K_AM_DR {
		...
		default:
			if cpu.Debug {
				fmt.Printf("M68K: Unimplemented extended addressing mode %d for MOVE at PC=%08x\n", srcReg, cpu.PC-M68K_WORD_SIZE)
			}
			cpu.ProcessException(M68K_VEC_ILLEGAL_INSTR)
			return
		}
		...
		if cpu.Debug {
			fmt.Printf("M68K: Unimplemented source mode %d for MOVE at PC=%08x\n", srcMode, cpu.PC-M68K_WORD_SIZE)
		}
		cpu.ProcessException(M68K_VEC_ILLEGAL_INSTR)
		return
	}
	...
	if destMode == M68K_AM_DR {
		...
		default:
			if cpu.Debug {
				fmt.Printf("M68K: Invalid destination mode 7, reg %d for MOVE at PC=%08x\n", destReg, cpu.PC-M68K_WORD_SIZE)
			}
			cpu.ProcessException(M68K_VEC_ILLEGAL_INSTR)
			return
		}
		...
		if cpu.Debug {
			fmt.Printf("M68K: Unimplemented destination mode %d for MOVE at PC=%08x\n", destMode, cpu.PC-M68K_WORD_SIZE)
		}
		cpu.ProcessException(M68K_VEC_ILLEGAL_INSTR)
		return
	}
...
func (cpu *M68KCPU) ExecMoveFromSR(mode, reg uint16) {
	...
	if mode == M68K_AM_DR {
		// Data register - store as word in low 16 bits
...
func (cpu *M68KCPU) ExecMovec() {
	...
	if direction == 0 {
		...
		default:
			if cpu.Debug {
				fmt.Printf("M68K: MOVEC to unknown control register %03X at PC=%08x\n", creg, cpu.PC-4)
			}
			cpu.ProcessException(M68K_VEC_ILLEGAL_INSTR)
			return
		}
		...
		default:
			if cpu.Debug {
				fmt.Printf("M68K: MOVEC from unknown control register %03X at PC=%08x\n", creg, cpu.PC-4)
			}
			cpu.ProcessException(M68K_VEC_ILLEGAL_INSTR)
			return
		}
...
func (cpu *M68KCPU) ExecMoveImmToAddr(opcode uint16) {
	...
	if (opcode & 0xF0C0) == 0x207C {
		...
		if cpu.Debug {
			fmt.Printf("MOVE immediate to A%d\n", reg)
		}
		...
		cpu.AddrRegs[reg] = uint32(value)
		if cpu.Debug {
			fmt.Printf("MOVE.W #0x%04X to A%d\n", uint16(value), reg)
		}
		cpu.cycleCounter += 8
	}
}
...
func (cpu *M68KCPU) ExecTst(size, mode, reg uint16) {
	if cpu.Debug {
		fmt.Printf("TST instruction: size=%d, mode=%d, reg=%d\n", size, mode, reg)
	}
	...
	if mode == M68K_AM_DR {
		...
		if cpu.Debug {
			fmt.Printf("TST on D%d: value=0x%X\n", reg, result)
		}
		cpu.cycleCounter += M68K_CYCLE_REG
		...
		if cpu.Debug {
			fmt.Printf("TST on A%d: value=0x%X\n", reg, result)
		}
		cpu.cycleCounter += M68K_CYCLE_REG
	} else {
		// Other addressing modes
		addr := cpu.GetEffectiveAddress(mode, reg)
		if cpu.Debug {
			fmt.Printf("TST effective address: 0x%08X\n", addr)
		}

		if sizeCode == M68K_SIZE_BYTE {
			result = uint32(cpu.Read8(addr))
			if cpu.Debug {
				fmt.Printf("TST.B at 0x%08X: Read value=0x%02X\n", addr, result)
			}
		} else if sizeCode == M68K_SIZE_WORD {
			result = uint32(cpu.Read16(addr))
			if cpu.Debug {
				fmt.Printf("TST.W at 0x%08X: Read value=0x%04X\n", addr, result)
			}
		} else { // Long
			result = cpu.Read32(addr)
			if cpu.Debug {
				fmt.Printf("TST.L at 0x%08X: Read value=0x%08X\n", addr, result)
			}
		}
		cpu.cycleCounter += M68K_CYCLE_MEM_READ
	}
	...
	if cpu.Debug {
		fmt.Printf("TST SR update: 0x%04X -> 0x%04X (Z flag: %v)\n", oldSR, cpu.SR, (cpu.SR&M68K_SR_Z) != 0)
	}
}
...
func (cpu *M68KCPU) ExecAdd(reg, opmode, mode, xreg uint16) {
	...
	if direction == M68K_DIRECTION_REG_TO_EA {
		...
		if mode == M68K_AM_DR {
			...
			// Address register indirect with displacement
			disp := int16(cpu.Fetch16())
			addr := cpu.AddrRegs[xreg] + uint32(disp)
			if size == M68K_SIZE_BYTE {
				dest = uint32(cpu.Read8(addr))
			} else if size == M68K_SIZE_WORD {
				dest = uint32(cpu.Read16(addr))
			...
			// Address register indirect with index
			extWord := cpu.Fetch16()
			addr := cpu.GetIndexWithExtWords(extWord, cpu.AddrRegs[xreg], true)
			if size == M68K_SIZE_BYTE {
				dest = uint32(cpu.Read8(addr))
			} else if size == M68K_SIZE_WORD {
				dest = uint32(cpu.Read16(addr))
	...
	if direction == M68K_DIRECTION_REG_TO_EA {
		...
		if mode == M68K_AM_DR {
			...
			// Postincrement - address was already incremented during read
			var adjustSize uint32
			if size == M68K_SIZE_BYTE {
				adjustSize = M68K_BYTE_SIZE
				if xreg == 7 {
					adjustSize = M68K_WORD_SIZE
				}
			} else if size == M68K_SIZE_WORD {
				adjustSize = M68K_WORD_SIZE
			} else {
				adjustSize = M68K_LONG_SIZE
			}
			addr := cpu.AddrRegs[xreg] - adjustSize
			if size == M68K_SIZE_BYTE {
				cpu.Write8(addr, uint8(result))
			} else if size == M68K_SIZE_WORD {
				cpu.Write16(addr, uint16(result))
			...
			// Predecrement - address was already decremented
			addr := cpu.AddrRegs[xreg]
			if size == M68K_SIZE_BYTE {
				cpu.Write8(addr, uint8(result))
			} else if size == M68K_SIZE_WORD {
			...
			// Displacement - recompute address
			disp := int16(cpu.Fetch16())
			addr := cpu.AddrRegs[xreg] + uint32(disp)
			if size == M68K_SIZE_BYTE {
				cpu.Write8(addr, uint8(result))
...
func (cpu *M68KCPU) ExecAnd(reg, opmode, mode, xreg uint16) {
	...
	if direction == M68K_DIRECTION_REG_TO_EA {
		...
		if mode == M68K_AM_DR {
			...
			// Postincrement - address was already incremented during read
			addr := cpu.AddrRegs[xreg] - (M68K_BYTE_SIZE * uint32(1<<size))
			if size == M68K_SIZE_BYTE {
				cpu.Write8(addr, uint8(result))
			} else if size == M68K_SIZE_WORD {
				cpu.Write16(addr, uint16(result))
...
func (cpu *M68KCPU) ExecAndi() {
	...
	// Skip CCR/SR variants as they're already implemented
	if mode == 0 && reg == 4 {
		return // ANDI to CCR
	}
	if mode == 0 && reg == 6 {
...
func (cpu *M68KCPU) ExecOr(reg, opmode, mode, xreg uint16) {
	...
	if direction == M68K_DIRECTION_REG_TO_EA {
		...
		if mode == M68K_AM_DR {
			...
			// Other addressing modes
			fmt.Printf("M68K: Unimplemented source mode %d for OR at PC=%08x\n", mode, cpu.PC-M68K_WORD_SIZE)
			cpu.ProcessException(M68K_VEC_ILLEGAL_INSTR)
			return
		}
		...
		if mode == M68K_AM_AR_IND {
			...
			// Other addressing modes
			fmt.Printf("M68K: Unimplemented destination mode %d for OR at PC=%08x\n", mode, cpu.PC-M68K_WORD_SIZE)
			cpu.ProcessException(M68K_VEC_ILLEGAL_INSTR)
			return
		}
	}
...
func (cpu *M68KCPU) ExecAbcd(rm, rx, ry uint16) {
	...
	// Get extend bit
	x := (cpu.SR & M68K_SR_X) != 0

	// Perform BCD addition
	lowNibble := (src & 0x0F) + (dst & 0x0F)
	if x {
		lowNibble++
	}
	...
	// Compute high nibble
	highNibble := ((src >> 4) & 0x0F) + ((dst >> 4) & 0x0F)
	if lowNibble > 0x0F {
		highNibble++
	}

	// Set flags
	cpu.SR &= ^uint16(M68K_SR_X | M68K_SR_C)
	...
	// Combine nibbles
	result = ((highNibble & 0x0F) << 4) | (lowNibble & 0x0F)
...
func (cpu *M68KCPU) ExecSbcd(rm, rx, ry uint16) {
	...
	// Process low nibble
	var lowResult int
	lowResult = int(dst&0xF) - int(src&0xF)
	if borrow {
		lowResult--
	}
	...
	// Process high nibble
	var highResult int
	highResult = int((dst>>4)&0xF) - int((src>>4)&0xF)
	if borrow {
		highResult--
	}
	...
	// Combine nibbles
	result = uint8(((highResult & 0xF) << 4) | (lowResult & 0xF))
...
func (cpu *M68KCPU) ExecNbcd(mode, reg uint16) {
	...
	// This is equivalent to SBCD with destination = 0
	borrow := x

	// Process low nibble: 0 - (src & 0xF) - borrow
	var lowResult int
	lowResult = 0 - int(src&0xF)
	if borrow {
		lowResult--
	}
	...
	// Process high nibble: 0 - (src >> 4) - borrow
	var highResult int
	highResult = 0 - int((src>>4)&0xF)
	if borrow {
		highResult--
	}
...
func (cpu *M68KCPU) ExecSTOP() {
	...
	cpu.Running = false
	if cpu.Debug {
		fmt.Printf("M68K: STOP encountered. Halting execution at PC=0x%08X\n", cpu.PC)
	}
}
...
func (cpu *M68KCPU) ExecBRA(opcode uint16) {
	...
	if condition == 0 { // BRA always branches.
		takeBranch = true
	} else if condition == 1 { // BSR: Branch to subroutine.
		cpu.Push32(cpu.PC)
		takeBranch = true
	} else {
	...
	if takeBranch {
		...
		if targetPC < M68K_ENTRY_POINT || targetPC >= M68K_MEMORY_SIZE-M68K_WORD_SIZE {
			if cpu.Debug {
				fmt.Printf("Error: Branch to invalid address 0x%08X\n", targetPC)
			}
			cpu.Running = false
			return
		}
...
func (cpu *M68KCPU) ExecBkpt(bkptNum uint16) {
	if cpu.Debug {
		fmt.Printf("M68K: BKPT #%d at PC=%08x\n", bkptNum, cpu.PC-M68K_WORD_SIZE)
	}
