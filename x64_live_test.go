...
func TestX64LiveDemoPayloadTargets(t *testing.T) {
	...
	for _, want := range []string{
		...
		`iewarp_service.asm`,
		`IEWarp runtime worker built: sdk/examples/prebuilt/iewarp_service.ie64`,
		...
		`test -f "$(AROS_ROM)"`,
		`test -f "$(AROS_RELEASE_DIR)/Devs/AHI/ie-audio.audio"`,
		...
		"aros-ie-live-assets:",
		"Building narrow IE AROS live assets...",
...
func TestAB3D2ReleaseTargetsBuildOnlyFourX64Binaries(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Guest-Advanced CRT filter is off by default. F7 cycles through flat, curved and off",
...
func TestX64LiveStagesDemoPayloadOnIESHARE(t *testing.T) {
	...
	for _, want := range []string{
		...
		`payload_require_file "${IEDOOM_IE86_PATH}" "make iedoom-ie86"`,
		`payload_require_file "${IEDOOM_IE68_PATH}" "make iedoom-ie68"`,
		`payload_require_file "${IEDOOM_WAD_PATH}" "copy DOOM1.WAD into CHOCOLATE_DOOM_DIR"`,
		...
		`Forbidden live payload content: non-user-facing prebuilt .ie* staged under IESHARE`,
		`Expected: coprocessor examples, bare-metal AROS duplicate demos, and smoke tests stay out of IESHARE`,
		`Forbidden legacy IEWarp worker path staged in live payload`,
		`Forbidden live payload content: embedded boot image staged under IESHARE`,
		`Expected: BASIC, EmuTOS, and AROS boot images are embedded in IntuitionEngine`,
		...
		`sha256sum -c "${host_sdk_sha256}"`,
		`Host SDK exceeds IESHARE payload budget`,
		`host-sdk-check.XXXXXX`,
		`test-installed-host-sdk.sh`,
