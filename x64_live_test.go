...
func TestX64LiveStagesDemoPayloadOnIESHARE(t *testing.T) {
	...
	for _, want := range []string{
		...
		`find "${payload_root}/Demos" -maxdepth 1 -type f -name 'ab3d2_*.ie68' ! -name '*redux_high*' ! -name '*redux_low*'`,
		...
		`python3 - "${AB3D2_EMBED_DIR}/_build.zip" "${AB3D2_EMBED_DIR}" "$demos_dir"`,
		...
		`Demos/ab3d2_ie68_redux_high.ie68`,
