// jit_store_helper_amd64_test.go — Phase 5 cycle 5.5: AMD64 STORE helper-exit.
...
func TestJIT_AMD64_STORE_HighAddr_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
...
func TestJIT_AMD64_STORE_MMUEnabled_SetsHelper(t *testing.T) {
	r := newJITTestRig(t)
...
// TestJIT_AMD64_STORE_HighAddr_UnmappedSrc_PreservesValue — covers the
// P1 regression where resolveRegAMD64 leaves an unmapped IE64 source
// register's value in R10, then the helper-exit emitter clobbered R10
// with the ctx pointer before writing HelperVal. Use R5 (spilled).
func TestJIT_AMD64_STORE_HighAddr_UnmappedSrc_PreservesValue(t *testing.T) {
	r := newJITTestRig(t)
	const highAddr uint64 = 0x0000_0001_0000_8000
	const payload uint64 = 0xA5A5A5A5A5A5A5A5
	r.cpu.regs[5] = payload
	r.cpu.regs[2] = highAddr
	r.ctx.NeedHelper = HELPER_NONE

	r.compileAndRun(t, ie64Instr(OP_STORE, 5, IE64_SIZE_Q, 0, 2, 0, 0))

	if r.ctx.NeedHelper != HELPER_STORE {
		t.Fatalf("NeedHelper = %d, want HELPER_STORE", r.ctx.NeedHelper)
	}
	if r.ctx.HelperVal != payload {
		t.Fatalf("HelperVal = 0x%016X, want 0x%016X (R10 clobber regression)", r.ctx.HelperVal, payload)
	}
}

func TestJIT_AMD64_STORE_LowAddr_NoHelper(t *testing.T) {
	r := newJITTestRig(t)
