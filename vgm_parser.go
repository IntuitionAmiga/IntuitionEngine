// vgm_parser.go - VGM/VGZ parser for PSG and SN76489 writes.
//
// Supported chips:
//   - AY-3-8910 / YM2149 (cmd 0xA0) - direct register mapping
//   - SN76489 / SN76496 (cmd 0x50) - raw latch/data bytes with timing
//
...
func ParseVGMData(data []byte) (*VGMFile, error) {
	...
	for i := int(dataStart); i < len(data); {
		...
		case cmd == 0x50:
			...
			snEvents = append(snEvents, SNEvent{Sample: samplePos, Byte: data[i+1]})
			i += 2
			continue
		case cmd == 0x62:
			samplePos += 735
	...
	lastEventSample := uint64(0)
	if len(events) > 0 {
		lastEventSample = max(lastEventSample, events[len(events)-1].Sample+1)
	}
	if len(snEvents) > 0 {
		lastEventSample = max(lastEventSample, snEvents[len(snEvents)-1].Sample+1)
	}
	if lastEventSample > 0 {
		last := max(totalSamples, lastEventSample)
		totalSamples = last
	}
