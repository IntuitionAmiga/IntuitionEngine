...
func TestIExec_M161_Phase4_GetIOSM_ExecLibrary(t *testing.T) {
	...
	if major, minor, patch := binary.LittleEndian.Uint16(got.manifest[10:12]), binary.LittleEndian.Uint16(got.manifest[12:14]), binary.LittleEndian.Uint16(got.manifest[14:16]); major != 1 || minor != 16 || patch != 6 {
		t.Fatalf("exec IOSM version=%d.%d.%d, want 1.16.6", major, minor, patch)
	}
