...
func TestIExec_M161_Phase7_HelpHeaderUsesIOSVersion(t *testing.T) {
	for _, rel := range []string{
		...
		if !bytes.Contains([]byte(body), []byte("IntuitionOS 1.16.6 help")) {
			t.Fatalf("%s help header must use IOS version 1.16.6, got:\n%s", rel, body)
		}
...
func TestIExec_M161_Phase7_VERSIONUsesIOSVersion(t *testing.T) {
	...
	if !bytes.Contains([]byte(body), []byte("IntuitionOS 1.16.6")) {
		t.Fatalf("VERSION command must use IOS version 1.16.6, got:\n%s", body)
	}
